...
// batch sign test ==========================
// not useful
// // fixed aux data len = 1024, vrf data len = 1024, batch size varies
// func BenchmarkBatchSign_1(b *testing.B)    { benchmarkBatchSignRingVRF(b, 1, 1024, 1024) }
// func BenchmarkBatchSign_10(b *testing.B)   { benchmarkBatchSignRingVRF(b, 10, 1024, 1024) }
// func BenchmarkBatchSign_100(b *testing.B)  { benchmarkBatchSignRingVRF(b, 100, 1024, 1024) }
// func BenchmarkBatchSign_1000(b *testing.B) { benchmarkBatchSignRingVRF(b, 1000, 1024, 1024) }

func benchmarkBatchSignRingVRF(b *testing.B, batch_size int, aux_data_len int, vrfinput_len int) {
	seeds := make([][]byte, 6)
	pubKeys := make([]BanderSnatchKey, 6)
	privateKeys := make([]BanderSnatchSecret, 6)
	for i := 0; i < 6; i++ {
	...
	// ramdom data aux_data and vrfinput
	aux_data_list := make([][]byte, batch_size)
	vrfinput_list := make([][]byte, batch_size)
	for i := 0; i < batch_size; i++ {
		...
		vrfinput_list[i] = vrfinput
	}
	b.ResetTimer()
	// Sign the data with Ring VRF using the third private key
	proverIdx := 2
	for i := 0; i < b.N; i++ {
		_, _, err := RingVrfBatchSign(privateKeys[proverIdx], ringSet, vrfinput_list, aux_data_list)
		if err != nil {
			b.Fatalf("RingVrfBatchSign failed: %v", err)
		}
	}
}
