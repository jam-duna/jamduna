...
pub(crate) mod serialization {
    use super::{
        ...
        Instance, InstanceQuery, Selector, VirtualCell,
    };
    use alloc::boxed::Box;
    use crate::collections::{vec, Vec};
    use crate::io::{self, Read, Write};
    use crate::plonk::{lookup, permutation};
    use crate::poly::Rotation;
    use ff::PrimeField;

    const EXPR_CONST: u8 = 0;
    const EXPR_SELECTOR: u8 = 1;
    const EXPR_FIXED: u8 = 2;
    const EXPR_ADVICE: u8 = 3;
    const EXPR_INSTANCE: u8 = 4;
    const EXPR_NEGATED: u8 = 5;
    const EXPR_SUM: u8 = 6;
    const EXPR_PRODUCT: u8 = 7;
    const EXPR_SCALED: u8 = 8;
    ...
    fn write_usize<W: Write>(w: &mut W, value: usize) -> io::Result<()> {
        let value = u32::try_from(value).map_err(|_| {
            io::Error::new(io::ErrorKind::Other, "usize exceeds u32")
        })?;
        write_u32(w, value)
    }
    ...
    fn read_field<F: PrimeField, R: Read>(r: &mut R) -> io::Result<F> {
        ...
        Option::from(F::from_repr(repr))
            .ok_or_else(|| io::Error::new(io::ErrorKind::Other, "invalid field encoding"))
    }

    fn write_rotation<W: Write>(w: &mut W, rotation: Rotation) -> io::Result<()> {
        write_i32(w, rotation.0)
    }

    fn read_rotation<R: Read>(r: &mut R) -> io::Result<Rotation> {
        Ok(Rotation(read_i32(r)?))
    }

    fn write_column_any<W: Write>(w: &mut W, column: Column<Any>) -> io::Result<()> {
        ...
        write_u8(w, kind)?;
        write_usize(w, column.index())
    }

    fn read_column_any<R: Read>(r: &mut R) -> io::Result<Column<Any>> {
        let kind = read_u8(r)?;
    ...
    fn write_selector<W: Write>(w: &mut W, selector: Selector) -> io::Result<()> {
        write_usize(w, selector.0)?;
        write_u8(w, if selector.1 { 1 } else { 0 })
    }

    fn read_selector<R: Read>(r: &mut R) -> io::Result<Selector> {
        let index = read_usize(r)?;
        let simple = read_u8(r)? == 1;
        Ok(Selector(index, simple))
    }

    fn write_query<W: Write>(w: &mut W, index: usize, column_index: usize, rotation: Rotation) -> io::Result<()> {
        write_usize(w, index)?;
        write_usize(w, column_index)?;
        write_rotation(w, rotation)
    }

    fn read_query<R: Read>(r: &mut R) -> io::Result<(usize, usize, Rotation)> {
        let index = read_usize(r)?;
        let column_index = read_usize(r)?;
        let rotation = read_rotation(r)?;
        Ok((index, column_index, rotation))
    }

    fn write_virtual_cell<W: Write>(w: &mut W, cell: &VirtualCell) -> io::Result<()> {
        write_column_any(w, cell.column)?;
        write_rotation(w, cell.rotation)
    }

    fn read_virtual_cell<R: Read>(r: &mut R) -> io::Result<VirtualCell> {
        let column = read_column_any(r)?;
        let rotation = read_rotation(r)?;
        Ok(VirtualCell { column, rotation })
    }

    pub(crate) fn write_expression<F: PrimeField, W: Write>(
        expr: &Expression<F>,
        w: &mut W,
    ) -> io::Result<()> {
        match expr {
            Expression::Constant(value) => {
                write_u8(w, EXPR_CONST)?;
                write_field(w, value)
            }
            Expression::Selector(selector) => {
                write_u8(w, EXPR_SELECTOR)?;
                write_selector(w, *selector)
            }
            Expression::Fixed(query) => {
                write_u8(w, EXPR_FIXED)?;
                write_query(w, query.index, query.column_index, query.rotation)
            }
            Expression::Advice(query) => {
                write_u8(w, EXPR_ADVICE)?;
                write_query(w, query.index, query.column_index, query.rotation)
            }
            Expression::Instance(query) => {
                write_u8(w, EXPR_INSTANCE)?;
                write_query(w, query.index, query.column_index, query.rotation)
            }
            Expression::Negated(inner) => {
                write_u8(w, EXPR_NEGATED)?;
                write_expression(inner, w)
            }
            Expression::Sum(a, b) => {
                write_u8(w, EXPR_SUM)?;
                write_expression(a, w)?;
                write_expression(b, w)
            }
            Expression::Product(a, b) => {
                write_u8(w, EXPR_PRODUCT)?;
                write_expression(a, w)?;
                write_expression(b, w)
            }
            Expression::Scaled(inner, scalar) => {
                write_u8(w, EXPR_SCALED)?;
                write_expression(inner, w)?;
                write_field(w, scalar)
            }
        }
    }

    pub(crate) fn read_expression<F: PrimeField, R: Read>(r: &mut R) -> io::Result<Expression<F>> {
        let tag = read_u8(r)?;
        match tag {
            EXPR_CONST => Ok(Expression::Constant(read_field(r)?)),
            EXPR_SELECTOR => Ok(Expression::Selector(read_selector(r)?)),
            EXPR_FIXED => {
                let (index, column_index, rotation) = read_query(r)?;
            ...
            EXPR_NEGATED => Ok(Expression::Negated(Box::new(read_expression(r)?))),
            EXPR_SUM => {
                let a = read_expression(r)?;
                let b = read_expression(r)?;
            ...
            EXPR_SCALED => {
                let inner = read_expression(r)?;
                let scalar = read_field(r)?;
                Ok(Expression::Scaled(Box::new(inner), scalar))
            }
            _ => Err(io::Error::new(io::ErrorKind::Other, "invalid expression tag")),
        }
    }

    pub(crate) fn write_constraint_system<F: PrimeField, W: Write>(
        cs: &ConstraintSystem<F>,
        w: &mut W,
    ) -> io::Result<()> {
        write_usize(w, cs.num_fixed_columns)?;
        write_usize(w, cs.num_advice_columns)?;
        write_usize(w, cs.num_instance_columns)?;
        write_usize(w, cs.num_selectors)?;

        write_usize(w, cs.selector_map.len())?;
        for col in cs.selector_map.iter() {
            write_usize(w, col.index())?;
        }

        write_usize(w, cs.gates.len())?;
        for gate in cs.gates.iter() {
            write_usize(w, gate.polys.len())?;
            for poly in gate.polys.iter() {
                write_expression(poly, w)?;
            }
            write_usize(w, gate.queried_selectors.len())?;
            for selector in gate.queried_selectors.iter() {
                write_selector(w, *selector)?;
            }
            write_usize(w, gate.queried_cells.len())?;
            for cell in gate.queried_cells.iter() {
                write_virtual_cell(w, cell)?;
            }
        }

        write_usize(w, cs.advice_queries.len())?;
        for (col, rotation) in cs.advice_queries.iter() {
            write_usize(w, col.index())?;
            write_rotation(w, *rotation)?;
        }

        write_usize(w, cs.num_advice_queries.len())?;
        for value in cs.num_advice_queries.iter() {
            write_usize(w, *value)?;
        }

        write_usize(w, cs.instance_queries.len())?;
        for (col, rotation) in cs.instance_queries.iter() {
            write_usize(w, col.index())?;
            write_rotation(w, *rotation)?;
        }

        write_usize(w, cs.fixed_queries.len())?;
        for (col, rotation) in cs.fixed_queries.iter() {
            write_usize(w, col.index())?;
            write_rotation(w, *rotation)?;
        }

        let perm_columns = cs.permutation.get_columns();
        write_usize(w, perm_columns.len())?;
        for column in perm_columns.iter() {
            write_column_any(w, *column)?;
        }

        write_usize(w, cs.lookups.len())?;
        for lookup_arg in cs.lookups.iter() {
            write_usize(w, lookup_arg.input_expressions.len())?;
            for expr in lookup_arg.input_expressions.iter() {
                write_expression(expr, w)?;
            }
            write_usize(w, lookup_arg.table_expressions.len())?;
            for expr in lookup_arg.table_expressions.iter() {
                write_expression(expr, w)?;
            }
        }

        write_usize(w, cs.constants.len())?;
        for col in cs.constants.iter() {
            write_usize(w, col.index())?;
        }

        match cs.minimum_degree {
            Some(value) => {
                write_u8(w, 1)?;
                write_usize(w, value)
            }
    ...
    pub(crate) fn read_constraint_system<F: PrimeField, R: Read>(
        r: &mut R,
    ) -> io::Result<ConstraintSystem<F>> {
        let num_fixed_columns = read_usize(r)?;
        let num_advice_columns = read_usize(r)?;
        let num_instance_columns = read_usize(r)?;
        let num_selectors = read_usize(r)?;

        let selector_map_len = read_usize(r)?;
        ...
        let gates_len = read_usize(r)?;
        let mut gates = Vec::with_capacity(gates_len);
        for _ in 0..gates_len {
            let polys_len = read_usize(r)?;
            let mut polys = Vec::with_capacity(polys_len);
            for _ in 0..polys_len {
                polys.push(read_expression(r)?);
            }

            let selectors_len = read_usize(r)?;
            let mut queried_selectors = Vec::with_capacity(selectors_len);
            for _ in 0..selectors_len {
                queried_selectors.push(read_selector(r)?);
            }

            let cells_len = read_usize(r)?;
            let mut queried_cells = Vec::with_capacity(cells_len);
            for _ in 0..cells_len {
                queried_cells.push(read_virtual_cell(r)?);
            }

            gates.push(super::Gate {
                name: "",
                constraint_names: vec![""; polys_len],
                polys,
                queried_selectors,
                queried_cells,
            });
        ...
        let advice_len = read_usize(r)?;
        let mut advice_queries = Vec::with_capacity(advice_len);
        for _ in 0..advice_len {
            let index = read_usize(r)?;
            let rotation = read_rotation(r)?;
            advice_queries.push((Column { index, column_type: Advice }, rotation));
        }

        let advice_counts_len = read_usize(r)?;
        let mut num_advice_queries = Vec::with_capacity(advice_counts_len);
        for _ in 0..advice_counts_len {
            num_advice_queries.push(read_usize(r)?);
        }

        let instance_len = read_usize(r)?;
        let mut instance_queries = Vec::with_capacity(instance_len);
        for _ in 0..instance_len {
            let index = read_usize(r)?;
            let rotation = read_rotation(r)?;
            instance_queries.push((Column { index, column_type: Instance }, rotation));
        }

        let fixed_len = read_usize(r)?;
        let mut fixed_queries = Vec::with_capacity(fixed_len);
        for _ in 0..fixed_len {
            let index = read_usize(r)?;
            let rotation = read_rotation(r)?;
            fixed_queries.push((Column { index, column_type: Fixed }, rotation));
        }

        let perm_len = read_usize(r)?;
        let mut perm_columns = Vec::with_capacity(perm_len);
        for _ in 0..perm_len {
            perm_columns.push(read_column_any(r)?);
        }
        let permutation = permutation::Argument::from_columns(perm_columns);

        let lookups_len = read_usize(r)?;
        let mut lookups = Vec::with_capacity(lookups_len);
        for _ in 0..lookups_len {
            let input_len = read_usize(r)?;
            let mut input_expressions = Vec::with_capacity(input_len);
            for _ in 0..input_len {
                input_expressions.push(read_expression(r)?);
            }

            let table_len = read_usize(r)?;
            let mut table_expressions = Vec::with_capacity(table_len);
            for _ in 0..table_len {
                table_expressions.push(read_expression(r)?);
            }

            lookups.push(lookup::Argument {
                input_expressions,
                table_expressions,
            });
        }

        let constants_len = read_usize(r)?;
        let mut constants = Vec::with_capacity(constants_len);
        for _ in 0..constants_len {
            let index = read_usize(r)?;
            constants.push(Column { index, column_type: Fixed });
        }

        let minimum_degree = match read_u8(r)? {
            0 => None,
            1 => Some(read_usize(r)?),
            _ => {
                return Err(io::Error::new(
