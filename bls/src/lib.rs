    ...
    for i in 0..k {
        ...
        for (j, &index) in indexes.iter().enumerate() {
            ...
            if (index as usize) < C {
                ...
                unsafe {
                    ptr::copy_nonoverlapping(shard.as_ptr(), output_ptr.add(offset), 2);
                }
            } else {
            ...
            match result {
                Ok(decoded) => {
                    for (index, segment) in decoded.restored_original_iter() {
                        ...
                        unsafe {
                            ptr::copy_nonoverlapping(segment.as_ptr(), output_ptr.add(offset), 2);
                        }
                    }
                }
                Err(_) => {
...
mod tests {
    ...
    fn encode_decode_test(d_hex: &str, V: usize, C: usize) -> Result<(), Box<dyn Error>> {
        ...
        for trial in 0..2 {
            ...
            println!("Trial {}: Selected indices: {:?}", trial, selected);

            let mut shard_buffer = vec![0u8; C * shard_size];
            let mut index_buffer = vec![0u32; C];
            for (i, &idx) in selected.iter().enumerate() {
                let shard = &encoded[idx * shard_size..(idx + 1) * shard_size];
                shard_buffer[i * shard_size..(i + 1) * shard_size].copy_from_slice(shard);
        ...
        assert!(success_count >= 2, "Too many failures");
        Ok(())
    }
    ...
    fn test_real_vectors() -> Result<(), Box<dyn Error>> {
        ...
        encode_decode_test(b268, 6, 2)?;
        encode_decode_test(s4104, 6, 2)?;
        Ok(())
    }
}
