...
func readSTF(targeted_mode string, t *testing.T) ([]*statedb.StateTransition, error) {
	stfs, err := readStateTransitions(targeted_mode)
	if err == nil {
		return stfs, nil
	}
	fmt.Printf("%v-mode specific data unavailable.. using STF Generic data instead", targeted_mode)
	stfs_generic, err := readStateTransitions(STF_Generic)
	if err != nil {
