# JAM Refine and Accumulate Tracing

This document explains a style of how to output JAM PVM trace logs flexibly, following through on a suggestion by Jan Bujak to use flat files for compression.  This has the advantage of having implementers share not giant
monolithic log files containing everything but streams, e.g. one for each register.   This is described in **Trace Streams**.  

As JAM `refine` and `accumulate` involve many PVM invocations (authorization and work items, ordered and parallel accumulation), it is natural to combine Trace Streams with hierarchical directory structures, keyed in by work package hash or accumulate time slot.  Within each directory, we may nest the Trace Streams, the invocation inputs (argument inputs and fetch inputs) and outputs (argument outputs and exports) as well as any derivations to support debugging.

This is a just a draft and a community effort, intended to save debugging time, to support JAM conformance.
...
* Files should use lowercased; If a file is compressed, the extension of the file should reveal the compression scheme (eg `.gz` stream is gzip-compressed binary data.) 
...
| `stores` | 12 bytes (`uint32` + `uint64`) | Length of that memory write (0 for both when no store occurred). |
...
- `<work package hash>/`  
  ...
  - `bundle`
  - `<workItemIndex>_<serviceId>/`: PVM trace data, one subfolder per work item containing a and 
      - `input`: refine input
...
Notes:
- Any `.bin` or `.json` extension imply JAM Codec and canonical JSON encoding, same as the W3F test vectors
...
We have compiled our trace for Doom here: [0.7.2 Doom 12/12/2025 - JAM DUNA](https://cdn.jamduna.org/doom.zip) - 1GB
...
Accumulation involves multiple parallel execution of multiple single accumulation. 
Using 0-based `ordered_accumulation_index`, we  have a simple subfolder per single service accumulation as follows:
...
**Example**: [0.7.2/fuzzy/00000151](https://github.com/jam-duna/jamtestnet/tree/main/jam-test-vectors/0.7.2)
