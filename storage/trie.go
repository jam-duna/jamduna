...
const (
	trieDebug          = "trie"
	trieStateKeySize   = 32 // the "actual" size is 31 but we use common.Hash with the 32 byte being 0 INCLUDING IN METADATA right now
...
// Bitmap represents 126 bits (16 bytes) for page-based storage
type Bitmap struct {
	data [16]byte
}
...
// Page represents a 4K page containing 126 nodes across 6 BMT layers
type Page struct {
	// Dirty tracking
	Dirty bool
...
type MerkleTree struct {
	Root          *Node
	backend       *JAMTrieBackend        // Shared across Copy instances
	stagedInserts map[common.Hash][]byte // Per-instance (not shared)
	stagedDeletes map[common.Hash]bool   // Per-instance (not shared)
	stagedMutex   sync.Mutex
	treeMutex     sync.RWMutex // Protects Root (per-instance)
	usePages      bool
	rootPage      *Page
}

func trieNormalizeKey32(src []byte) common.Hash {
...
func NewMerkleTree(data [][2][]byte) *MerkleTree {
	return NewMerkleTreeWithBackend(data, nil)
}

func NewMerkleTreeWithBackend(data [][2][]byte, backend *JAMTrieBackend) *MerkleTree {
	if backend == nil {
	...
	t.Root = t.buildMerkleTree(data, 0)
	return t
}

// Copy creates an isolated view sharing the backend but with its own Root and staged ops.
...
func (t *MerkleTree) cloneNode(n *Node) *Node {
	...
	return &Node{Hash: n.Hash, Key: n.Key, Left: n.Left, Right: n.Right}
}
...
func (t *MerkleTree) buildMerkleTree(kvs [][2][]byte, i int) *Node {
	...
	if len(kvs) == 1 {
		encoded := trieLeaf(kvs[0][0], kvs[0][1], t)
		t.levelDBSetLeaf(encoded, kvs[0][1], kvs[0][0])
		return &Node{Hash: trieComputeHash(encoded), Key: kvs[0][0]}
	}
	...
	encoded := trieBranch(left.Hash, right.Hash)
	branchHash := trieComputeHash(encoded)
...
func trieBranch(left, right []byte) []byte {
	if len(left) != 32 || len(right) != 32 {
		log.Crit(trieDebug, "branch: input hashes must be 32 bytes")
	}
...
func trieLeaf(k, v []byte, t *MerkleTree) []byte {
	var result [64]byte
...
func trieDecodeLeaf(leaf []byte) (k []byte, v []byte, isEmbedded bool, err error) {
	if len(leaf) != 64 {
...
func trieBit(k []byte, i int) bool {
...
func (t *MerkleTree) GetRoot() common.Hash {
	// For page-based storage, flush if dirty
	if t.usePages {
		...
		if len(t.stagedInserts) > 0 || len(t.stagedDeletes) > 0 || (t.rootPage != nil && t.rootPage.Dirty) {
			...
			if err != nil {
				...
				return trieNormalizeKey32(make([]byte, 32))
			}
			return trieNormalizeKey32(root[:])
		}

		if t.rootPage == nil {
			return trieNormalizeKey32(make([]byte, 32))
		}
		return trieNormalizeKey32(t.rootPage.SubRoot[:])
	}
...
func (t *MerkleTree) SetRoot(root common.Hash) error {
	...
	writeBatchSize, rootExistsInBatch := t.backend.GetBatchInfo(root)
	log.Trace(log.SDB, "🔍 MerkleTree.SetRoot: attempting to load root",
		"root", root.Hex()[:16],
...
// InitMerkleTreeFromHashPageBased creates a MerkleTree with page-based storage from a root hash
func InitMerkleTreeFromHashPageBased(rootHash common.Hash, db types.JAMStorage) (*MerkleTree, error) {
	if db == nil {
		return nil, fmt.Errorf("database is not initialized")
	}
...
func (t *MerkleTree) Flush() (common.Hash, error) {
	// Use page-based storage if enabled
	if t.usePages {
		return t.flushPageBased()
	}
	...
	if numOps == 0 {
		...
		for key, value := range inserts {
			...
			encodedLeaf := trieLeaf(keySlice, value, t)
			...
			if err != nil {
				...
				// Update existing
				node.Hash = trieComputeHash(trieLeaf(keySlice, value, t))
...
// flushPageBased applies staged operations to page-based storage
func (t *MerkleTree) flushPageBased() (common.Hash, error) {
	t.stagedMutex.Lock()
	inserts := t.stagedInserts
	deletes := t.stagedDeletes
	...
	for keyHash := range deletes {
		...
		if err := t.rootPage.Delete(key, 0, nil); err != nil { // TODO: Pass actual db
	...
	for keyHash, value := range inserts {
		...
		if err := t.rootPage.Insert(key, value, 0, nil); err != nil { // TODO: Pass actual db
			return common.Hash{}, err
		}
	}
	...
	if err := t.rootPage.Flush(nil); err != nil { // TODO: Pass actual db
		return common.Hash{}, err
	}
...
func (t *MerkleTree) levelDBSetLeaf(encodedLeaf, value []byte, key []byte) {
	_k, _v, isEmbedded, _ := trieDecodeLeaf(encodedLeaf)
	encodedLeafHash := trieComputeHash(encodedLeaf)
	encodedLeafHashVal := trieComputeHash(append(encodedLeafHash, value...))
...
func (t *MerkleTree) getDBLeafValue(nodeHash []byte) ([]byte, bool, error) {
	...
	_, _v, isEmbedded, err := trieDecodeLeaf(encodedLeaf)
	if err != nil {
...
func (t *MerkleTree) levelDBSet(k, v []byte) error {
	...
	copy(keyHash[:], k)
	t.backend.PutNode(keyHash, v)
	return nil
}

// levelDBGetLocked is the internal version that assumes backend lock is already held
func (t *MerkleTree) levelDBGetLocked(k []byte) ([]byte, bool, error) {
	var keyHash common.Hash
	copy(keyHash[:], k)
	return t.backend.GetNodeLocked(keyHash)
}
...
func (t *MerkleTree) levelDBGet(k []byte) ([]byte, bool, error) {
	var keyHash common.Hash
	copy(keyHash[:], k)
	return t.backend.GetNode(keyHash)
}
...
func (t *MerkleTree) levelDBGetNode(nodeHash common.Hash) (*Node, error) {
	...
	if leafValue != nil {
		...
		encodedLeafHashVal := trieComputeHash(append(nodeHash[:], leafValue...))
...
func (t *MerkleTree) GetRealKey(key [31]byte, value []byte) []byte {
	encodedLeaf := trieLeaf(key[:], value, t)
	nodeHash := trieComputeHash(encodedLeaf)
...
func (t *MerkleTree) GetStates() ([16][]byte, error) {
	var states [16][]byte
	for i := uint8(0); i < 16; i++ {
		stateKey := make([]byte, trieStateKeySize)
		stateKey[0] = i + 1 // State indices are 1-16
...
func (t *MerkleTree) Insert(keyBytes []byte, value []byte) {
	key := trieNormalizeKey32(keyBytes)
...
func (t *MerkleTree) insertNode(node *Node, key, value []byte, depth int) *Node {
	...
	if node == nil || trieCompareBytes(node.Hash, nullNode.Hash) || depth > trieComputeKeyLengthAsBit(key) {
		return &Node{
			Hash: trieComputeHash(trieLeaf(key, value, t)),
			Key:  key,
		}
	}

	if node.Left == nil && node.Right == nil {
		if trieCompareBytes(node.Key, key) {
			node.Hash = trieComputeHash(trieLeaf(key, value, t))
			return node
		}
		return t.createBranchNode(node, key, value, depth)
	}

	if trieBit(key, depth) {
	...
	node.Hash = trieComputeHash(trieBranch(leftHash, rightHash))
...
func (t *MerkleTree) createBranchNode(node *Node, key, value []byte, depth int) *Node {
	...
	if trieBit(existingKey, depth) {
		node.Right = &Node{
			Hash: trieComputeHash(trieLeaf(existingKey, existingValue, t)),
			Key:  existingKey,
		}
	} else {
		node.Left = &Node{
...
func (t *MerkleTree) updateTree(node *Node, key, value []byte, depth int) {
	if node == nil || depth > trieComputeKeyLengthAsBit(key) {
		return
	}
	if trieCompareBytes(node.Key, key) {
		node.Hash = trieComputeHash(trieLeaf(key, value, t))
...
func (t *MerkleTree) Get(keyBytes []byte) ([]byte, bool, error) {
	...
	if t.usePages {
		...
		var key31 [31]byte
		copy(key31[:], key[:31])
		return t.rootPage.Get(key31, 0, nil) // TODO: Pass actual db
	}
...
func trieLeafStandalone(k, v []byte) []byte {
	var result [64]byte
...
func Verify(serviceID uint32, key []byte, value []byte, rootHash []byte, path []common.Hash) bool {
	...
	if len(path) == 0 {
		leafHashSingle := trieComputeHash(trieLeafStandalone(opaqueKey, value))
		return trieCompareBytes(leafHashSingle, rootHash)
	}

	leafHash := trieComputeHash(trieLeafStandalone(opaqueKey, value))

	for i := len(path) - 1; i >= 0; i-- {
		if trieBit(opaqueKey, i) {
			leafHash = trieComputeHash(trieBranch(path[i][:], leafHash))
		} else {
			leafHash = trieComputeHash(trieBranch(leafHash, path[i][:]))
		}
	}
	return trieCompareBytes(leafHash, rootHash)
}
...
func VerifyRaw(key []byte, value []byte, rootHash []byte, path []common.Hash) bool {
	if len(path) == 0 {
		leafHashSingle := trieComputeHash(trieLeafStandalone(key, value))
		return trieCompareBytes(leafHashSingle, rootHash)
	}

	leafHash := trieComputeHash(trieLeafStandalone(key, value))
...
func (t *MerkleTree) deleteNode(node *Node, key []byte, depth int) *Node {
	...
	if node.Key != nil && trieCompareBytes(node.Key, key) {
...
func CompareTrees(node1, node2 *Node) bool {
	return trieCompareTrees(node1, node2)
}

func trieCompareTrees(node1, node2 *Node) bool {
	...
	if !trieCompareBytes(node1.Hash, node2.Hash) {
		fmt.Printf("Node Hash Mismatch N1=%x N2=%x\n", node1.Hash, node2.Hash)
		fmt.Printf("Node1 %v\n", node1.String())
		fmt.Printf("Node2 %v\n", node2.String())
		return false
	}
	if !trieCompareBytes(node1.Key, node2.Key) {
		fmt.Printf("Node Key Mismatch N1=%x N2=%x\n", node1.Key, node2.Key)
		fmt.Printf("Node1 %v\n", node1.String())
		fmt.Printf("Node2 %v\n", node2.String())
		return false
	}
	return trieCompareTrees(node1.Left, node2.Left) && trieCompareTrees(node1.Right, node2.Right)
}

// Compute the key length in bits. byte -> 8bits
func trieComputeKeyLengthAsBit(key []byte) int {
	return len(key) * 8
}
