...
func trieCompareBytes(a, b []byte) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
...
func trieComputeHash(data []byte, hashType ...string) []byte {
	var h hash.Hash
...
func trieComputeNode(data []byte, hashType ...string) []byte {
	var h hash.Hash
	// Check if "keccak" is passed in hashType, else use default Blake2b-256.
	if len(hashType) > 0 && hashType[0] == types.Keccak {
		h = sha3.NewLegacyKeccak256()
...
func trieComputeLeaf(data []byte, hashType ...string) []byte {
	var h hash.Hash
	// Check if "keccak" is passed in hashType, else use default Blake2b-256.
	if len(hashType) > 0 && hashType[0] == types.Keccak {
		h = sha3.NewLegacyKeccak256()
...
func ComputeLeaf(data []byte, hashType ...string) []byte {
	return trieComputeLeaf(data, hashType...)
}
