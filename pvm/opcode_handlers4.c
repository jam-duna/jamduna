...
void handle_STORE_IMM_U8(VM* vm, uint8_t* operands, size_t operand_len) {
    ...
    uint64_t err_code = vm_write_ram_bytes_8(vm, addr, (uint8_t)vy);
    if (err_code != OK) {
...
#if PVM_TRACE
        dump_store_generic("STORE_IMM_U8", (uint64_t)addr, "imm", vy & 0xff, 8);
#endif
...
#if PVM_TRACE
        dump_store_generic("STORE_IMM_U16", (uint64_t)addr, "imm", (uint64_t)value, 16);
#endif
...
#if PVM_TRACE
        dump_store_generic("STORE_IMM_U32", (uint64_t)addr, "imm", (uint64_t)value, 32);
#endif
...
#if PVM_TRACE
        dump_store_generic("STORE_IMM_U64", (uint64_t)addr, "imm", vy, 64);
#endif
