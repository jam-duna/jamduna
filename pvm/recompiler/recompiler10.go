...
func generateRotateRight32Imm(inst Instruction) []byte {
	...
	if srcIdx == dstIdx {
		...
		// 2) ROR r/m32(dst), imm8 - use 32-bit shift helper
		code = append(code, emitShiftRegImm32(dst, X86_OP_GROUP2_RM_IMM8, X86_REG_ROR, byte(imm&X86_SHIFT_MASK_32))...)
	...
	// 1) MOV r32_dst, r32_src - truncates to 32-bit
	code = append(code, emitMovRegToReg32(dst, src)...)
	// 2) ROR r/m32(dst), imm8 - use 32-bit shift helper
	code = append(code, emitShiftRegImm32(dst, X86_OP_GROUP2_RM_IMM8, X86_REG_ROR, byte(imm&X86_SHIFT_MASK_32))...)
	// 3) MOVSXD r64_dst, r32_dst - sign-extend to 64-bit
	code = append(code, emitMovsxd64(dst, dst)...)
...
func generateImmShiftOp32Alt(subcode byte) func(inst Instruction) []byte {
	return func(inst Instruction) []byte {
		...
		// 1) Load 64-bit immediate
		// 2) Shift 64-bit
		// 3) Truncate to 32-bit (AND with 0xFFFFFFFF)
		...
		// 2) MOV RCX, src64 (put shift count in CL) - MUST do this BEFORE loading imm to dst!
		if src.Name != "RCX" {
		...
		// 6) Sign-extend 32-bit result to 64-bit: MOVSXD r64, r/m32
		code = append(code, emitMovsxd64(dst, dst)...)
