...
var pvmByteCodeToX86Code = map[byte]func(Instruction) []byte{
	...
	ROT_R_64_IMM_ALT:  generateImmShiftOp64Alt(X86_REG_ROR),
...
// emitBswap64 emits: BSWAP dst, src (64-bit byte swap)
// Note: BSWAP only operates on a single register, so we need MOV first if src != dst
func emitBswap64(dst, src X86Reg) []byte {
	var code []byte

	// If src and dst are different, first move src to dst
	...
	code = append(code, rex, X86_PREFIX_0F, opcode)
...
// emitShiftRegCl32 emits: D3 /subcode r/m32, CL (32-bit shift with CL register)
func emitShiftRegCl32(dst X86Reg, subcode byte) []byte {
...
// emitAndImmToReg64 emits: AND r/m64, imm32 (0x81 /4 id)
func emitAndImmToReg64(reg X86Reg, imm uint32) []byte {
	rex := buildREX(true, false, false, reg.REXBit == 1)
	modrm := buildModRM(X86_MOD_REGISTER, 4, reg.RegBits) // subcode 4 for AND
	result := []byte{rex, 0x81, modrm}
	result = append(result, encodeU32(imm)...)
	return result
}
...
// emitDivMemStackRemUOp32RDX emits: DIV dword ptr [RSP] (for src2==RDX case in 32-bit)
...
// emitIdivMemStackRemSOp32RDX emits: IDIV dword ptr [RSP] (for src2==RDX case in 32-bit signed)
