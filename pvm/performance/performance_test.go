...
// CompilerCompareStats holds comparison stats between Go and C compilers
type CompilerCompareStats struct {
	FileName       string
	PVMInstrCount  int
	GoCompileTime  time.Duration
	CCompileTime   time.Duration
	Speedup        float64 // C speedup vs Go (Go/C ratio)
}

// TestBenchmarkCompareGoAndC compares compile time between Go and C compilers
func TestBenchmarkCompareGoAndC(t *testing.T) {
	pattern := filepath.Join("..", "..", "services", "*", "*.pvm")
	matches, err := filepath.Glob(pattern)
	if err != nil {
		t.Fatalf("Glob failed: %v", err)
	...
	var allStats []CompilerCompareStats
	var totalGoTime, totalCTime time.Duration

	// Print header
	fmt.Println("=" + strings.Repeat("=", 99))
	fmt.Printf("%-35s %10s %12s %12s %10s\n",
		"File", "PVM Instr", "Go Time", "C Time", "Speedup")
	fmt.Println("-" + strings.Repeat("-", 99))
	...
	for _, path := range matches {
		...
		// Benchmark Go compiler
		compilerGo := recompiler.NewX86Compiler(code)
		if err := compilerGo.SetBitMask(bitmask); err != nil {
			t.Errorf("Go SetBitMask failed for %s: %v", name, err)
			continue
		}
		if err := compilerGo.SetJumpTable(jumpTable); err != nil {
			t.Errorf("Go SetJumpTable failed for %s: %v", name, err)
			continue
		}
		startGo := time.Now()
		compilerGo.CompileX86Code(0)
		goTime := time.Since(startGo)

		// Benchmark C compiler
		compilerC := recompiler_c.NewC_Compiler(code)
		if err := compilerC.SetBitMask(bitmask); err != nil {
			t.Errorf("C SetBitMask failed for %s: %v", name, err)
			continue
		}
		if err := compilerC.SetJumpTable(jumpTable); err != nil {
			t.Errorf("C SetJumpTable failed for %s: %v", name, err)
			continue
		}
		startC := time.Now()
		compilerC.CompileX86Code(0)
		cTime := time.Since(startC)

		// Calculate speedup
		speedup := float64(goTime) / float64(cTime)

		stats := CompilerCompareStats{
			FileName:      name,
			PVMInstrCount: pvmInstrCount,
			GoCompileTime: goTime,
			CCompileTime:  cTime,
			Speedup:       speedup,
		}
		allStats = append(allStats, stats)
		...
		// Print result
		fmt.Printf("%-35s %10d %12s %12s %9.2fx\n",
	...
	// Print summary
	fmt.Println("-" + strings.Repeat("-", 99))
	totalSpeedup := float64(totalGoTime) / float64(totalCTime)
	fmt.Printf("%-35s %10s %12s %12s %9.2fx\n",
		"TOTAL",
		"",
		totalGoTime,
		totalCTime,
	...
	fmt.Println("\nSummary:")
	fmt.Printf("  Total Go compile time:   %s\n", totalGoTime)
	fmt.Printf("  Total C compile time:    %s\n", totalCTime)
	fmt.Printf("  C compiler speedup:      %.2fx faster\n", totalSpeedup)
}
