...
void handle_ECALLI(VM* vm, uint8_t* operands, size_t operand_len) {
    ...
    vm->gas -= (int64_t)vm_charge_gas(vm, vm->host_func_id);
...
void handle_LOAD_IMM_64(VM* vm, uint8_t* operands, size_t operand_len) {
...
#if PVM_TRACE
        printf("LOAD_IMM_64 DEBUG: reg=%d, operand_len=%zu, need=%d\n", 
               register_index_a, operand_len, 1 + lx);
#endif
    ...
    #if PVM_TRACE
            printf("LOAD_IMM_64: Reading 8 bytes from code, ignoring operand_len=%zu\n", operand_len);
#endif
    } else {
    ...
    #if PVM_TRACE
            printf("LOAD_IMM_64 WARNING: Not enough code bytes, partial load from %zu operand bytes\n", available_bytes);
#endif
    }
    
#if PVM_TRACE
        printf("LOAD_IMM_64: Setting register[%d] = 0x%llx\n", register_index_a, (unsigned long long)vx);
#endif
...
#if PVM_TRACE
        dump_jump_offset("JUMP", vx, vm->pc);
#endif
    
    vm_branch(vm, (uint64_t)((int64_t)vm->pc + vx), 1);
}
