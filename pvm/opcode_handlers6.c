...
// Helper function to print operands for debugging
static void print_operands(const char* opcode, uint8_t* operands, size_t operand_len) {
    printf("%s DEBUG: operand_len=%zu, operands=[", opcode, operand_len);
...
#if PVM_TRACE
        dump_branch_imm("JUMP_IND", register_index_a, value_a, vx, target, 0, 1);
#endif
...
#if PVM_TRACE
        dump_load_imm("LOAD_IMM", register_index_a, vx, vx, 64, 0);
#endif
...
#if PVM_TRACE
        dump_load_generic("LOAD_U8", register_index_a, (uint64_t)addr, result, 8, 0);
#endif
...
#if PVM_TRACE
        dump_load_generic("LOAD_I8", register_index_a, (uint64_t)addr, result, 8, 1);
#endif
...
#if PVM_TRACE
        dump_load_generic("LOAD_U16", register_index_a, (uint64_t)addr, result, 16, 0);
#endif
...
#if PVM_TRACE
        dump_load_generic("LOAD_I16", register_index_a, (uint64_t)addr, result, 16, 1);
#endif
...
#if PVM_TRACE
        dump_load_generic("LOAD_U32", register_index_a, (uint64_t)addr, result, 32, 0);
#endif
...
#if PVM_TRACE
        dump_load_generic("LOAD_I32", register_index_a, (uint64_t)addr, result, 32, 1);
#endif
...
void handle_LOAD_U64(VM* vm, uint8_t* operands, size_t operand_len) {
    ...
    uint32_t addr = (uint32_t)addr_64;
    int err_code;
    uint64_t value = vm_read_ram_bytes_64(vm, addr, &err_code);
    if (err_code != OK) {
        vm_panic(vm, err_code);
        return;
    }
...
#if PVM_TRACE
        dump_load_generic("LOAD_U64", register_index_a, (uint64_t)addr, value, 64, 0);
#endif
...
void handle_STORE_U8(VM* vm, uint8_t* operands, size_t operand_len) {
    ...
    uint32_t addr = (uint32_t)addr_64;
    uint64_t value_a = vm->registers[register_index_a];
    uint64_t err_code = vm_write_ram_bytes_8(vm, addr, (uint8_t)value_a);
    if (err_code != OK) {
        vm_panic(vm, err_code);
        return;
    }
...
#if PVM_TRACE
        dump_store_generic("STORE_U8", (uint64_t)addr, reg_name(register_index_a), value_a & 0xff, 8);
#endif
...
#if PVM_TRACE
        dump_store_generic("STORE_U16", (uint64_t)addr, reg_name(register_index_a), (uint64_t)value, 16);
#endif
...
#if PVM_TRACE
        dump_store_generic("STORE_U32", (uint64_t)addr, reg_name(register_index_a), (uint64_t)value, 32);
#endif
...
void handle_STORE_U64(VM* vm, uint8_t* operands, size_t operand_len) {
    ...
    uint32_t addr = (uint32_t)addr_64;
    uint64_t value_a = vm->registers[register_index_a];
    uint64_t err_code = vm_write_ram_bytes_64(vm, addr, value_a);
    if (err_code != OK) {
        vm_panic(vm, err_code);
        return;
    }
...
#if PVM_TRACE
        dump_store_generic("STORE_U64", (uint64_t)addr, reg_name(register_index_a), value_a, 64);
#endif
