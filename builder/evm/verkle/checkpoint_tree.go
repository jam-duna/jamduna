...
// EvmBlockPayload represents the block structure needed for checkpoint replay
type EvmBlockPayload struct {
	VerkleRoot       [32]byte
	VerkleStateDelta *VerkleStateDelta
}
...
type CheckpointTreeManager struct {
	...
	blockLoader func(uint64) (*EvmBlockPayload, error)
}
...
func (cm *CheckpointTreeManager) replayRange(
	...
	for h := baseHeight + 1; h <= targetHeight; h++ {
		...
		if block.VerkleStateDelta != nil && block.VerkleStateDelta.NumEntries > 0 {
			// Convert evmtypes.VerkleStateDelta to verkle.VerkleStateDelta
			verkleDelta := &VerkleStateDelta{
			...
			replayedTree, err := replayVerkleStateDelta(currentTree, verkleDelta, expectedRoot)
			if err != nil {
