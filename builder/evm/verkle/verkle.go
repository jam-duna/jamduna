...
func GenerateVerkleProof(tree verkle.VerkleNode, keys [][]byte) ([]byte, [][]byte, [][]byte, error) {
	// For read-only proofs, pass nil for post-tree to indicate no state transition
	// Actually, go-verkle requires both trees - use tree.Copy() to avoid same pointer
	treeCopy := tree.Copy()
	return GenerateVerkleProofWithTransition(tree, treeCopy, keys)
}
...
func GenerateVerkleProofWithTransition(preTree verkle.VerkleNode, postTree verkle.VerkleNode, keys [][]byte) ([]byte, [][]byte, [][]byte, error) {
	...
	// Debug: surface which stems/suffixes are requested (helps align with Rust verifier)
	{
		stemMap := make(map[[31]byte][]byte)
		for _, k := range keys {
			var stem [31]byte
			copy(stem[:], k[:31])
			stemMap[stem] = append(stemMap[stem], k[31])
		}
		for stem, suffixes := range stemMap {
			sort.Slice(suffixes, func(i, j int) bool { return suffixes[i] < suffixes[j] })
			log.Info(log.EVM, "🔑 Go proof keys (stem → suffixes)",
				"stem", fmt.Sprintf("%x", stem[:4]),
				"suffixes", fmt.Sprintf("%v", suffixes))
		}
	}
	...
	// Collect pre-values from pre-tree (used for debugging self-check below)
	preValues := make([][]byte, len(keys))
	for i, key := range keys {
	...
	// Commit trees BEFORE proof generation
	// This ensures all commitments are computed before MakeVerkleMultiProof
	preRootBytes := preTree.Commit().Bytes()
	postRootBytes := postTree.Commit().Bytes()

	log.Trace(log.EVM, "GenerateVerkleProofWithTransition: generating proof",
		"numKeys", len(keys),
		"preRoot", fmt.Sprintf("%x", preRootBytes[:8]),
		"postRoot", fmt.Sprintf("%x", postRootBytes[:8]))
	...
	proof, commitments, indices, frValues, err := verkle.MakeVerkleMultiProof(preTree, postTree, keys, nil)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("GenerateVerkleProofWithTransition: MakeVerkleMultiProof failed: %w", err)
	}

	log.Info(log.EVM, "🔍 GO MULTIPROOF ELEMENTS",
		"commitments", len(commitments),
		"indices", len(indices),
		"values", len(frValues))
	for i := 0; i < len(commitments) && i < 20; i++ {
		commBytes := commitments[i].Bytes()
		valBytes := frValues[i].Bytes()
		valBytesLE := frValues[i].BytesLE()
		log.Info(log.EVM, "  🔎 Go element",
			"i", i,
			"z", indices[i],
			"C", fmt.Sprintf("[%02x,%02x,%02x,%02x]", commBytes[0], commBytes[1], commBytes[2], commBytes[3]),
			"y", fmt.Sprintf("[%02x,%02x,%02x,%02x]", valBytes[0], valBytes[1], valBytes[2], valBytes[3]),
			"y_le", fmt.Sprintf("[%02x,%02x,%02x,%02x]", valBytesLE[0], valBytesLE[1], valBytesLE[2], valBytesLE[3]))
	}
	...
	// Marshal the ORIGINAL proof (without prepended root) for Go self-check.
	origProofBytes, err := marshalCompactProof(vp)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("GenerateVerkleProofWithTransition: compact proof marshaling failed: %w", err)
	}
	if ok, verifyErr := VerifyVerkleProofWithTransition(origProofBytes, preRootBytes[:], postRootBytes[:], keys, preValues, values); verifyErr != nil {
		log.Warn(log.EVM, "🧪 Go self-verification (orig proof) failed", "err", verifyErr)
	} else {
		log.Info(log.EVM, "🧪 Go self-verification (orig proof)", "ok", ok)
	}

	// CRITICAL: Prepend post-state root to CommitmentsByPath for Rust verifier
	// - go-verkle's MakeVerkleMultiProof intentionally EXCLUDES the root from CommitmentsByPath
	//   (assumes it's known from the block header)
	...
	// - This ensures mismatched roots still fail verification
	postRootCommitment := postTree.Commit().Bytes()

	newCommitments := make([][32]byte, 0, len(vp.CommitmentsByPath)+1)
	var rootArray [32]byte
	copy(rootArray[:], postRootCommitment[:])
	newCommitments = append(newCommitments, rootArray)
	newCommitments = append(newCommitments, vp.CommitmentsByPath...)

	vp.CommitmentsByPath = newCommitments

	// Log proof structure BEFORE marshaling
	log.Info(log.EVM, "📊 GO PROOF STRUCTURE (before marshaling)",
		"other_stems_count", len(vp.OtherStems),
		"depth_extension_present_len", len(vp.DepthExtensionPresent),
		"commitments_by_path_count", len(vp.CommitmentsByPath),
		"ipa_proof_depth", len(vp.IPAProof.CL),
		"D", fmt.Sprintf("%x", vp.D[:]))

	if len(vp.OtherStems) > 0 {
		log.Info(log.EVM, "  📌 other_stems[0]", "stem", fmt.Sprintf("%x", vp.OtherStems[0][:]))
	}
	if len(vp.CommitmentsByPath) > 0 {
		log.Info(log.EVM, "  📌 commitments[0] (ROOT)", "commitment", fmt.Sprintf("%x", vp.CommitmentsByPath[0][:]))
	}
	if len(vp.CommitmentsByPath) > 1 {
		log.Info(log.EVM, "  📌 commitments[1]", "commitment", fmt.Sprintf("%x", vp.CommitmentsByPath[1][:]))
	}

	// Log depth bytes in detail
	for i, depthByte := range vp.DepthExtensionPresent {
		depth := depthByte >> 3
		stemType := depthByte & 0x07
		log.Info(log.EVM, "  📏 depth byte",
			"index", i,
			"raw", fmt.Sprintf("0x%02x", depthByte),
			"depth", depth,
			"type", stemType)
	}

	// Marshal VerkleProof to compact binary bytes (matches Rust verifier format)
	proofBytes, err := marshalCompactProof(vp)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("GenerateVerkleProofWithTransition: compact proof marshaling failed: %w", err)
	}

	log.Info(log.EVM, "📦 GO MARSHALED PROOF", "total_bytes", len(proofBytes))
...
func VerifyVerkleProof(proof []byte, root []byte, keys [][]byte, values [][]byte) (bool, error) {
	...
	// Unmarshal VerkleProof from compact bytes
	vp, err := unmarshalCompactProof(proof)
	if err != nil {
