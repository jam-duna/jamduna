...
func ReportVerifyTiny(jsonFile string, exceptErr error) error {
...
func ReportVerifyFull(jsonFile string, exceptErr error) error {
	jsonPath := filepath.Join("../jamtestvectors/reports/full", jsonFile)
...
func TestReportVerifyTiny(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.jsonFile, func(t *testing.T) {
			err := ReportVerifyTiny(tc.jsonFile, tc.except)
			if err != nil {
...
func TestReportVerifyFull(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.jsonFile, func(t *testing.T) {
			err := ReportVerifyFull(tc.jsonFile, tc.except)
			if err != nil {
				t.Fatalf("\033[31mReports FAIL: %v\033[0m", err)
			} else {
				fmt.Printf("\033[32mReports PASS: %s\033[0m\n", tc.jsonFile)
...
func TestReportVerifyFullFailed(t *testing.T) {
	// run through all the json files in the tiny folder
	testCases := []struct {
		jsonFile string
		except   error
...
// These cases should NOT return any errorduring STF check
func TestReportValidReportCaseFull(t *testing.T) {
	testCases := []struct {
		jsonFile string
		except   error
		message  string
