...
func (stateDB *StateDB) GetTransactionReceipt(serviceID uint32, txHash common.Hash) (*EthereumTransactionReceipt, error) {
...
func (stateDB *StateDB) EstimateGas(from common.Address, to *common.Address, gas uint64, gasPrice uint64, value uint64, data []byte, pvmBackend string) (uint64, error) {
	...
	workReport, err := stateDB.executeSimulationWorkPackage(wpq, pvmBackend)
	if err != nil {
...
func (stateDB *StateDB) Call(from common.Address, to *common.Address, gas uint64, gasPrice uint64, value uint64, data []byte, blockNumber string, pvmBackend string) ([]byte, error) {
	...
	// Execute simulation
	wr, err := stateDB.executeSimulationWorkPackage(wpq, pvmBackend)
	if err != nil {
...
func (stateDB *StateDB) createSimulationWPQueueItem(tx *EthereumTransaction) (*types.WPQueueItem, error) {
	...
	wpq := &types.WPQueueItem{
		...
		Extrinsics:  types.ExtrinsicsBlobs{extrinsic},
	}
	return wpq, nil
}

// executeSimulationWorkPackage executes a work package for transaction simulation
func (stateDB *StateDB) executeSimulationWorkPackage(wpq *types.WPQueueItem, pvmBackend string) (workReport *types.WorkReport, err error) {
