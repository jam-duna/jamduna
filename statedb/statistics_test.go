...
func testStaticsSTF(t *testing.T, jsonFile string) {
	jsonPath := path.Join(common.GetJAMTestVectorPath("stf"), "statistics/", jsonFile)
	...
	jam_state.SafroleState.CurrValidators = prestate.CurrValidators_prime
	jam_state.SafroleState.Timeslot = timeslot
	...
	epoch_change := (slot / types.EpochLength) > (timeslot / types.EpochLength)
	...
	post_jam := state{}
	post_jam.Timeslot = prestate.Timeslot // should be: input.Slot
	...
	post_jam.CurrValidators_prime = JamState.SafroleState.CurrValidators
