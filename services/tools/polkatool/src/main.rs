...
fn main() {
    ...
    let result = match args {
        ...
        Args::JAMService {
            ...
            is_authorize,
            for_deblob,
        } => main_jam_service(input, output, dump, is_authorize, for_deblob),
    };
...
fn main_disassemble(
    ...
    let mut disassembler = polkavm_disassembler::Disassembler::new(&blob, format)
...
fn main_jam_service(
    ...
    if is_authorize {
        config.set_dispatch_table(vec![b"is_authorize".into()]);
    } else if for_deblob {
        config.set_dispatch_table(vec![b"main".into()]);
    } else {
        config.set_dispatch_table(vec![b"refine".into(), b"accumulate".into()]);
    }
    ...
    let _bitmask = blob.bitmask();

    println!("o_size: {:?}, w_size: {:?}, z: {:?}, s: {:?}, o_byte: [... {} bytes], c_size: {:?}, c (code_and_jump_table): ArcBytes {{ data: [... {} bytes] }}",
        o_size, w_size, z, s, o_byte.len(), c_size, c.len());
    // println!("\nbitmask: {:?}", bitmask);
