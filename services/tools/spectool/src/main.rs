...
fn extract_chunks(base_address: u32, slice: &[u8]) -> Vec<MemoryChunk> {
    ...
    let mut position = 0;
    while let Some(next_position) = slice[position..]
        .iter()
...
fn parse_pre_post(line: &str, output: &mut PrePost) {
    ...
    let index = line
        .find('=')
        .expect("invalid 'pre' / 'post' directive: no '=' found");
    ...
    if lhs == "gas" {
        ...
        let lhs = polkavm_common::utils::parse_reg(lhs)
            .expect("invalid 'pre' / 'post' directive: failed to parse lhs");
...
fn main_generate() {
    ...
    for testcase in testcases {
        ...
        let expected_final_pc = if let Some(export) = blob
            .exports()
        ...
        } else if let Some(ProgramCounterRef::ByLabel {
            label,
            instruction_offset,
        }) = post.pc
        {
            let Some(export) = blob
                .exports()
                .find(|export| export.symbol().as_bytes() == label.as_bytes())
            else {
        ...
        if module_config.dynamic_paging() {
            ...
            for chunk in &initial_memory {
                instance
                    .write_memory(chunk.address, &chunk.contents)
                    .unwrap();
            }
        }
        ...
        for ((final_value, reg), required_value) in
            expected_regs.iter().zip(Reg::ALL).zip(post.regs.iter())
        {
