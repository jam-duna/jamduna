...
const ABI_SUPPORT_COMMON_PRIVATE_EXTERA_REGISTERS_RS: &str =
    include_str!("abi_support_impl/common_private_extra_registers.rs");
...
pub fn polkavm_impl_abi_support(attributes: AbiSupportAttributes) -> proc_macro2::TokenStream {
    let common_public_code: proc_macro2::TokenStream =
        ABI_SUPPORT_COMMON_PUBLIC_RS.parse().unwrap();
    let common_private_code: proc_macro2::TokenStream =
        ABI_SUPPORT_COMMON_PRIVATE_RS.parse().unwrap();
    let common_private_extra_registers_code: proc_macro2::TokenStream =
        ABI_SUPPORT_COMMON_PRIVATE_EXTERA_REGISTERS_RS
            .parse()
            .unwrap();
