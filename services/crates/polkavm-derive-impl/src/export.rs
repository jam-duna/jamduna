...
pub fn polkavm_export(
    attributes: ExportBlockAttributes,
    input: syn::ItemFn,
    ...
    let mut remaining_variables = ["a", "b", "c", "d", "e", "f"].into_iter().map(|ident| {
        crate::common::expr_from_ident(syn::Ident::new(ident, proc_macro2::Span::call_site()))
    });
