...
fn ident_from_pattern_strict(
    syn::PatType { attrs, pat, .. }: &syn::PatType,
...
impl syn::parse::Parse for ImportAttribute {
    fn parse(input: syn::parse::ParseStream) -> syn::parse::Result<Self> {
        ...
        if lookahead.peek(kw::symbol) {
            ...
            let value = value
                .base10_parse::<u32>()
                .map_err(|err| syn::Error::new(value.span(), err))?;
...
fn parse_import_attributes(
    attr: &syn::Attribute,
) -> Result<Option<Vec<ImportAttribute>>, syn::Error> {
...
pub fn polkavm_import(
    attributes: ImportBlockAttributes,
    input: syn::ItemForeignMod,
    ...
    for item in input.items {
        match item {
            syn::ForeignItem::Fn(syn::ForeignItemFn {
                ...
                let abi_path = attributes
                    .abi
                    .clone()
                    .unwrap_or_else(crate::common::default_abi_path);
