...
fn test_sign_ext() {
    ...
    assert_eq!(
        sign_unext(0b11111111111111111111111111111101, 3),
        Some(0b101)
    );
...
impl Inst {
    ...
    fn decode_compressed(config: &DecoderConfig, op: u32) -> Option<Self> {
        ...
        match (quadrant, funct3) {
            ...
            (0b01, 0b011) if Reg::decode(op >> 7) == Reg::SP && op & 0b00010000_01111100 != 0 => {
                Some(Inst::RegImm {
                    kind: xlen!(RegImmKind, Add32, Add64),
                    dst: Reg::SP,
                    src: Reg::SP,
            ...
            (0b01, 0b011) if Reg::decode(op >> 7) != Reg::Zero && op & 0b00010000_01111100 != 0 => {
    ...
    pub fn decode(config: &DecoderConfig, op: u32) -> Option<Self> {
        ...
        match op & 0b1111111 {
            ...
            0b0110011 => {
                ...
                let kind = match op & 0b1111111_00000_00000_111_00000_0000000 {
                    ...
                    0b0000000_00000_00000_001_00000_0000000 => {
                        xlen!(RegRegKind, ShiftLogicalLeft32, ShiftLogicalLeft64)
                    }
                    0b0000000_00000_00000_010_00000_0000000 => {
                        xlen!(RegRegKind, SetLessThanSigned32, SetLessThanSigned64)
                    }
                    0b0000000_00000_00000_011_00000_0000000 => {
                        xlen!(RegRegKind, SetLessThanUnsigned32, SetLessThanUnsigned64)
                    }
                    0b0000000_00000_00000_100_00000_0000000 => xlen!(RegRegKind, Xor32, Xor64),
                    0b0000000_00000_00000_101_00000_0000000 => {
                        xlen!(RegRegKind, ShiftLogicalRight32, ShiftLogicalRight64)
                    }
                    0b0100000_00000_00000_101_00000_0000000 => {
                        xlen!(RegRegKind, ShiftArithmeticRight32, ShiftArithmeticRight64)
                    }
                    ...
                    0b0000001_00000_00000_001_00000_0000000 => {
                        xlen!(RegRegKind, MulUpperSignedSigned32, MulUpperSignedSigned64)
                    }
                    0b0000001_00000_00000_010_00000_0000000 => xlen!(
                        RegRegKind,
                        MulUpperSignedUnsigned32,
                        MulUpperSignedUnsigned64
                    ),
                    0b0000001_00000_00000_011_00000_0000000 => xlen!(
                        RegRegKind,
                        MulUpperUnsignedUnsigned32,
                        MulUpperUnsignedUnsigned64
                    ),
                    0b0000001_00000_00000_100_00000_0000000 => xlen!(RegRegKind, Div32, Div64),
                    0b0000001_00000_00000_101_00000_0000000 => {
                        xlen!(RegRegKind, DivUnsigned32, DivUnsigned64)
                    }
                    0b0000001_00000_00000_110_00000_0000000 => xlen!(RegRegKind, Rem32, Rem64),
                    0b0000001_00000_00000_111_00000_0000000 => {
                        xlen!(RegRegKind, RemUnsigned32, RemUnsigned64)
                    }
                    ...
                    0b0110000_00000_00000_001_00000_0000000 => {
                        xlen!(RegRegKind, RotateLeft32, RotateLeft64)
                    }
                    0b0110000_00000_00000_101_00000_0000000 => {
                        xlen!(RegRegKind, RotateRight32, RotateRight64)
                    }
            ...
            0b0111011 => {
                ...
                let kind = match op & 0b1111111_00000_00000_111_00000_0000000 {
                    0b0000000_00000_00000_000_00000_0000000 if config.rv64 => {
                        RegRegKind::Add32AndSignExtend
                    }
                    0b0000000_00000_00000_001_00000_0000000 if config.rv64 => {
                        RegRegKind::ShiftLogicalLeft32AndSignExtend
                    }
                    0b0000000_00000_00000_101_00000_0000000 if config.rv64 => {
                        RegRegKind::ShiftLogicalRight32AndSignExtend
                    }

                    0b0000001_00000_00000_000_00000_0000000 if config.rv64 => {
                        RegRegKind::Mul32AndSignExtend
                    }
                    0b0000001_00000_00000_100_00000_0000000 if config.rv64 => {
                        RegRegKind::Div32AndSignExtend
                    }
                    0b0000001_00000_00000_101_00000_0000000 if config.rv64 => {
                        RegRegKind::DivUnsigned32AndSignExtend
                    }
                    0b0000001_00000_00000_110_00000_0000000 if config.rv64 => {
                        RegRegKind::Rem32AndSignExtend
                    }
                    0b0000001_00000_00000_111_00000_0000000 if config.rv64 => {
                        RegRegKind::RemUnsigned32AndSignExtend
                    }

                    0b0100000_00000_00000_000_00000_0000000 if config.rv64 => {
                        RegRegKind::Sub32AndSignExtend
                    }
                    0b0100000_00000_00000_101_00000_0000000 if config.rv64 => {
                        RegRegKind::ShiftArithmeticRight32AndSignExtend
                    }

                    0b0110000_00000_00000_001_00000_0000000 => {
                        RegRegKind::RotateLeft32AndSignExtend
                    }
                    0b0110000_00000_00000_101_00000_0000000 => {
                        RegRegKind::RotateRight32AndSignExtend
                    }
...
mod test_decode_compressed {
    ...
    fn illegal_instruction() {
        ...
        assert_eq!(
            Inst::decode_compressed(&config, 1 << 16),
            Some(Inst::Unimplemented)
        );

        assert_eq!(
            Inst::decode_compressed(&config64, 1 << 16),
            Some(Inst::Unimplemented)
        );
    }
    ...
    fn test_bits_imm_c_jump() {
        assert_eq!(
            bits_imm_c_jump(0b001_10101010101_01),
            0b11111111_11111111_11111110_10100100
        );
