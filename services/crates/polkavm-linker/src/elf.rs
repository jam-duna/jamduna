    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    ...
    pub fn is_undefined(&self) -> bool {
        matches!(
            self.elf_symbol.section(),
            object::read::SymbolSection::Undefined
        )
    }

    pub fn section_and_offset(&self) -> Result<(&Section<'_>, u64), ProgramFromElfError> {
        ...
        let Some(offset) = self
            ...
            return Err(ProgramFromElfError::other(
                "relative symbol address underflow",
            ));
        };
    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    pub fn parse(data: &'data [u8]) -> Result<Self, ProgramFromElfError> {
        ...
        if elf.elf_header().e_ident().data != object::elf::ELFDATA2LSB {
            return Err(ProgramFromElfError::other(
                "file is not a little endian ELF file",
            ));
        }
        ...
        if os_abi != object::elf::ELFOSABI_SYSV && os_abi != object::elf::ELFOSABI_GNU {
            return Err(ProgramFromElfError::other(
                "file doesn't use the System V nor GNU ABI",
            ));
        }
        ...
        if elf.elf_header().e_machine(LittleEndian) != object::elf::EM_RISCV {
            return Err(ProgramFromElfError::other(
                "file is not a RISC-V file (EM_RISCV)",
            ));
        }
        ...
        for section in elf.sections() {
            ...
            let file_start = usize::try_from(file_range.0).map_err(|_| {
                ProgramFromElfError::other(format!("out of range offset for '{name}' section"))
            })?;

            let file_size = usize::try_from(file_range.1).map_err(|_| {
                ProgramFromElfError::other(format!("out of range size for '{name}' section"))
            })?;

            let file_end = file_start.checked_add(file_size).ok_or_else(|| {
                ProgramFromElfError::other(format!("out of range '{name}' section (overflow)"))
            })?;

            let file_range = file_start..file_end;
            let file_data = data.get(file_range).ok_or_else(|| {
                ProgramFromElfError::other(format!(
                    "out of range '{name}' section (out of bounds of ELF file)"
                ))
            })?;
            ...
            if section.elf_section_header().sh_type(LittleEndian) == object::elf::SHT_RELA {
                ...
                while struct_offset < section_data.len() {
                    ...
                    if is_64_bit {
                        ...
                        let rela = unsafe {
                            rela.as_ptr()
                                .cast::<object::elf::Rela64<LittleEndian>>()
                                .read_unaligned()
                        };
                        ...
                        let rela = unsafe {
                            rela.as_ptr()
                                .cast::<object::elf::Rela32<LittleEndian>>()
                                .read_unaligned()
                        };
            ...
            if section_index_map.insert(section.index(), index).is_some() {
                return Err(ProgramFromElfError::other(
                    "multiple sections with the same section index",
                ));
            }
        }
        ...
        for section_index in (0..sections.len()).map(SectionIndex) {
            ...
            if let Some(relocation_sections) =
                relocation_sections_for_section.remove(&elf_section_index.0)
            {
    ...
    ) -> Result<Symbol<'_, '_, H>, object::Error> {
        self.raw_elf
            .as_ref()
            .unwrap()
    ...
    pub fn section_by_name(&self, name: &str) -> impl ExactSizeIterator<Item = &Section<'_>> {
