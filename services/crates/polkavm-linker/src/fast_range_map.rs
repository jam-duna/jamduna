...
impl<V> RangeMap<V> {
    ...
    pub fn remove(&mut self, key: Range<u64>) -> RemoveIter<'_, V>
    where
        V: Clone,
    {
    ...
    pub fn reserve(&mut self, size: usize) {
        self.data.nodes.reserve(size, Node::hasher);
    }
    ...
    pub fn get_in_range(&self, range: Range<u64>) -> impl Iterator<Item = (Range<u64>, &V)> {
        self.get_all_overlapping(range.clone())
            .map(move |(key, value)| {
                (
                    core::cmp::max(range.start, key.start)..core::cmp::min(range.end, key.end),
                    value,
                )
            })
    }
    ...
    V: Clone,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        Some((chopped_key, value))
    }
}

impl<V> Extend<(Range<u64>, V)> for RangeMap<V>
where
    V: Clone,
{
    fn extend<T>(&mut self, iter: T)
    where
        T: IntoIterator<Item = (Range<u64>, V)>,
    {
...
impl<V> FromIterator<(Range<u64>, V)> for RangeMap<V>
where
    V: Clone,
{
