use polkavm_common::abi::{
    MemoryMapBuilder, VM_CODE_ADDRESS_ALIGNMENT, VM_MAX_PAGE_SIZE, VM_MIN_PAGE_SIZE,
};
use polkavm_common::cast::cast;
use polkavm_common::program::{
    self, FrameKind, Instruction, InstructionSet, LineProgramOp, Opcode, ProgramBlob,
...
polkavm_common::static_assert!(
    Reg::INPUT_REGS.len() == polkavm_common::program::Reg::MAXIMUM_INPUT_REGS
);
polkavm_common::static_assert!(
    Reg::OUTPUT_REGS.len() == polkavm_common::program::Reg::MAXIMUM_OUTPUT_REGS
);
...
impl core::fmt::Display for ProgramFromElfError {
    fn fmt(&self, fmt: &mut core::fmt::Formatter) -> core::fmt::Result {
        match &self.0 {
            ProgramFromElfErrorKind::FailedToParseElf(error) => {
                write!(fmt, "failed to parse ELF file: {}", error)
            }
            ProgramFromElfErrorKind::FailedToParseDwarf(error) => {
                write!(fmt, "failed to parse DWARF: {}", error)
            }
            ProgramFromElfErrorKind::FailedToParseProgram(error) => write!(fmt, "{}", error),
            ProgramFromElfErrorKind::UnsupportedSection(section) => {
                write!(fmt, "unsupported section: {}", section)
            }
            ...
            ProgramFromElfErrorKind::UnsupportedRegister { reg } => {
                write!(fmt, "unsupported register: {reg}")
            }
...
fn cast_reg_non_zero(reg: RReg) -> Result<Option<Reg>, ProgramFromElfError> {
    ...
    match reg {
        ...
        A4 => Ok(Some(Reg::A4)),
        A5 => Ok(Some(Reg::A5)),
        GP | TP | A6 | A7 | S2 | S3 | S4 | S5 | S6 | S7 | S8 | S9 | S10 | S11 | T3 | T4 | T5
        | T6 => Err(ProgramFromElfErrorKind::UnsupportedRegister { reg }.into()),
    }
}
...
impl SourceStack {
    ...
    fn display(&self, section_to_function_name: &BTreeMap<SectionTarget, &str>) -> String {
        ...
        for source in &self.0 {
            ...
            if let Some((origin, name)) = section_to_function_name
                .range(..=source.begin())
                .next_back()
            {
                if origin.section_index == source.section_index {
                    write!(
                        &mut out,
                        " \"{name}\"+{}",
                        source.offset_range.start - origin.offset
                    )
                    .unwrap();
                }
            }
...
fn test_extract_delimited() {
    ...
    assert_eq!(
        extract_delimited(&mut str, "<section #", ">").unwrap(),
        ("foo ", "1234+567")
    );
    assert_eq!(str, " bar");
}
                ...
                if let Some(offset) = iter.next().and_then(|s| s.parse::<u64>().ok()) {
                    if let Some(section) = elf.section_by_raw_index(section_index) {
                        ...
                        let symbol = elf.symbols().find(|symbol| {
                            let Ok((symbol_section, symbol_offset)) = symbol.section_and_offset()
                            else {
                                return false;
                            };
                        ...
                        write!(
                            &mut output,
                            "<section #{section_index}+{offset} ('{section_name}'"
                        )
                        .unwrap();
...
impl<T> BasicInst<T> {
    ...
    fn src_mask(&self, imports: &[Import]) -> RegMask {
        match *self {
            ...
            BasicInst::Cmov { dst, src, cond, .. } => {
                RegMask::from(dst) | RegMask::from(src) | RegMask::from(cond)
            }
            BasicInst::Ecalli { nth_import } => imports[nth_import].src_mask(),
            BasicInst::Sbrk { size, .. } => RegMask::from(size),
            BasicInst::Memset => {
                RegMask::from(Reg::A0) | RegMask::from(Reg::A1) | RegMask::from(Reg::A2)
            }
        }
    }
    ...
    fn dst_mask(&self, imports: &[Import]) -> RegMask {
        match *self {
            BasicInst::Nop | BasicInst::StoreAbsolute { .. } | BasicInst::StoreIndirect { .. } => {
                RegMask::empty()
            }
    ...
    fn has_side_effects(&self, config: &Config) -> bool {
        match *self {
            ...
            BasicInst::LoadAbsolute { .. } | BasicInst::LoadIndirect { .. } => {
                !config.elide_unnecessary_loads
            }
            BasicInst::Nop
            | BasicInst::LoadHeapBase { .. }
            | BasicInst::MoveReg { .. }
    ...
    fn map_register(self, mut map: impl FnMut(Reg, OpKind) -> Reg) -> Option<Self> {
        ...
        match self {
            ...
            BasicInst::LoadAddressIndirect { dst, target } => {
                Some(BasicInst::LoadAddressIndirect {
                    dst: map(dst, OpKind::Write),
                    target,
                })
            }
        ...
        T: Clone,
    {
        ...
        if is_special_instruction {
            ...
            let BasicInst::Ecalli { nth_import } = *self else {
                unreachable!()
            };
    ...
    fn map_target<U, E>(self, map: impl Fn(T) -> Result<U, E>) -> Result<BasicInst<U>, E> {
        Ok(match self {
            ...
            BasicInst::LoadAddress { dst, target } => BasicInst::LoadAddress {
                dst,
                target: map(target)?,
            },
            BasicInst::LoadAddressIndirect { dst, target } => BasicInst::LoadAddressIndirect {
                dst,
                target: map(target)?,
            },
            BasicInst::LoadIndirect {
    ...
    {
        match self {
            BasicInst::LoadAbsolute { target, .. } | BasicInst::StoreAbsolute { target, .. } => {
                (Some(*target), None)
            }
            BasicInst::LoadAddress { target, .. }
            | BasicInst::LoadAddressIndirect { target, .. } => (None, Some(*target)),
            BasicInst::Nop
            | BasicInst::LoadHeapBase { .. }
            | BasicInst::MoveReg { .. }
            | BasicInst::LoadImmediate { .. }
...
impl<T> ControlInst<T> {
    fn src_mask(&self) -> RegMask {
        match *self {
            ControlInst::Jump { .. } | ControlInst::Call { .. } | ControlInst::Unimplemented => {
                RegMask::empty()
            }
            ControlInst::JumpIndirect { base, .. } | ControlInst::CallIndirect { base, .. } => {
                RegMask::from(base)
            }
    ...
    fn dst_mask(&self) -> RegMask {
        match *self {
            ...
            ControlInst::Call { ra, .. } | ControlInst::CallIndirect { ra, .. } => {
                RegMask::from(ra)
            }
        }
    }
    ...
    fn map_target<U, E>(self, map: impl Fn(T) -> Result<U, E>) -> Result<ControlInst<U>, E> {
        Ok(match self {
            ControlInst::Jump { target } => ControlInst::Jump {
                target: map(target)?,
            },
            ControlInst::Call {
                ra,
    ...
    fn targets(&self) -> [Option<&T>; 2] {
        match self {
            ...
            ControlInst::Call {
                ...
                target_return,
                ..
            } => [Some(target), Some(target_return)],
        ...
        T: Copy,
    {
        match self {
            ControlInst::Jump { .. }
            | ControlInst::JumpIndirect { .. }
            | ControlInst::Unimplemented => None,
            ControlInst::Branch {
                target_false: target,
                ..
            }
    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    ...
    *current_address =
        align_to_next_page_u64(u64::from(VM_MAX_PAGE_SIZE), *current_address).expect("overflow");
    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    ...
    let rw_data_size = process_sections(
        ...
        sections_rw_data
            .iter()
            .copied()
            .chain(sections_bss.iter().copied()),
    );
    ...
    let min_stack_size = align_to_next_page_u32(VM_MIN_PAGE_SIZE, min_stack_size).ok_or(
        ProgramFromElfError::other("out of range size for the stack"),
    )?;
    ...
    loop {
        ...
        let metadata = {
            ...
            let address = if elf.is_64() {
                ...
                b.read_u32().map(u64::from)
            };
            let address = address.map_err(|error| {
                ProgramFromElfError::other(format!("failed to parse export metadata: {}", error))
            })?;
    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    ...
    if version != 1 && version != 2 {
        return Err(format!(
            "unsupported extern metadata version: '{version}' (expected '1' or '2')"
        ));
    }
    ...
    let symbol_location = match symbol_relocation {
        ...
        _ => {
            return Err(format!(
                "unexpected relocation for the symbol: {symbol_relocation:?}"
            ))
        }
    };
    ...
    let Some(symbol) = elf
        ...
        .data()
        .get(
            symbol_location.offset as usize
                ..symbol_location
                    .offset
                    .saturating_add(u64::from(symbol_length)) as usize,
        )
    else {
...
where
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    parse_extern_metadata_impl(elf, relocations, target).map_err(|error| {
        ProgramFromElfError::other(format!("failed to parse extern metadata: {}", error))
    })
}

fn check_imports_and_assign_indexes(
    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    match relocation.target() {
        ...
        object::RelocationTarget::Symbol(target_symbol_index) => {
            let target_symbol = elf.symbol_by_index(target_symbol_index).map_err(|error| {
                ProgramFromElfError::other(format!("failed to fetch relocation target: {}", error))
            })?;
...
fn resolve_simple_zero_register_usage(
    ...
    if matches!(
        kind,
        K::RotateLeft32AndSignExtend | K::RotateRight32AndSignExtend
    ) && src1 != RReg::Zero
        && src2 == RReg::Zero
    {
        emit(InstExt::Basic(BasicInst::AnyAny {
...
fn emit_or_combine_byte(
    ...
    log::warn!(
        "Emulating orc.b at {:?} with an instruction sequence",
        location
    );
    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    ...
    while relative_offset < text.len() {
        ...
        if let (crate::riscv::OPCODE_CUSTOM_0, FUNC3_SBRK, 0, dst, size, RReg::Zero) =
            crate::riscv::R(raw_inst).unpack()
        {
            let Some(dst) = cast_reg_non_zero(dst)? else {
                return Err(ProgramFromElfError::other(
            ...
            output.push((
                Source {
                    section_index,
                    offset_range: (relative_offset as u64..relative_offset as u64 + inst_size)
                        .into(),
                },
        ...
        if let (crate::riscv::OPCODE_CUSTOM_0, FUNC3_HEAP_BASE, 0, dst, RReg::Zero, RReg::Zero) =
            crate::riscv::R(raw_inst).unpack()
        {
            output.push((
                Source {
        ...
        let source = Source {
            section_index,
            offset_range: AddressRange::from(
                relative_offset as u64..relative_offset as u64 + inst_size,
            ),
        };
        ...
        if let Some(inst) = instruction_overrides.remove(&current_location) {
            ...
            } = original_inst
            {
                if relative_offset < text.len() {
                    ...
                    if let Some(Inst::JumpAndLinkRegister {
                        dst: ra_dst,
                        base,
                        value,
                    }) = next_inst
                                ...
                                if offset >= 0 && offset < section.data().len() as i32 {
                                    output.push((
                                        ...
                                        InstExt::Control(ControlInst::Call {
                                            ...
                                            target_return: current_location
                                                .add(inst_size + next_inst_size),
                                        }),
                                    ));
    ...
    for (source, op) in instructions {
        ...
        if let Some(last_source_in_block) = last_source_in_block {
            ...
            if source == last_source_in_block {
                let InstExt::Basic(instruction) = op else {
                    unreachable!()
                };
            ...
            if let Some((block_section, block_start)) = block_start_opt.take() {
                ...
                if !current_block.is_empty() {
                    ...
                    let end_of_block_source = Source {
                        section_index: block_section,
                        offset_range: (last_instruction_source.offset_range.start
                            ..source.offset_range.start)
                            .into(),
                    };
                    ...
                    assert!(
                        end_of_block_source.offset_range.start
                            < end_of_block_source.offset_range.end
                    );

                    log::trace!(
                        "Emitting block (due to a potential jump): {}",
                        block_source.begin()
                    );
        ...
        match op {
            InstExt::Control(instruction) => {
                ...
                log::trace!(
                    "Emitting block (due to a control instruction): {}",
                    block_source.begin()
                );
                blocks.push(BasicBlock::new(
                    block_index,
                ...
                if let ControlInst::Branch { target_false, .. } = instruction {
                    if !cfg!(test) {
                        if source.section_index != target_false.section_index {
                            return Err(ProgramFromElfError::other(
                                "found a branch with a fallthrough to another section",
                            ));
                        }
...
fn build_section_to_block_map(
    ...
    for (block_index, block) in blocks.iter().enumerate() {
        ...
        if section_to_block
            .insert(section_target, block_target)
            .is_some()
        {
            return Err(ProgramFromElfError::other(
                "found two or more basic blocks with the same location",
            ));
        }
    }
...
fn garbage_collect_reachability(
    ...
    while !queue_code.is_empty() || !queue_data.is_empty() {
        ...
        while let Some(data_target) = queue_data.pop_unique() {
            if let Some(list) = reachability_graph
    ...
    if set_code.len() == reachability_graph.for_code.len()
        && set_data.len() == reachability_graph.for_data.len()
    {
        return false;
    }
    ...
    reachability_graph
        .for_code
        .retain(|block_target, reachability| {
            reachability
                .reachable_from
                .retain(|inner_key| set_code.contains(inner_key));
            reachability
                .address_taken_in
                .retain(|inner_key| set_code.contains(inner_key));
            reachability
                .referenced_by_data
                .retain(|inner_key| set_data.contains(inner_key));
    ...
    reachability_graph
        .for_data
        .retain(|data_target, reachability| {
    ...
    reachability_graph
        .code_references_in_data_section
        .retain(|data_target, list| {
            if !set_data.contains(data_target) {
                false
            } else {
                assert!(list
                    .iter()
                    .all(|block_target| set_code.contains(block_target)));
                true
            }
        });
    ...
    reachability_graph
        ...
        .retain(|data_target, list| {
            if !set_data.contains(data_target) {
                ...
                assert!(list
                    .iter()
                    .all(|next_data_target| set_data.contains(next_data_target)));
                true
            }
        });
...
fn remove_unreachable_data_impl(
    ...
    let code_refs = reachability_graph
        .code_references_in_data_section
        .remove(&current);
    let data_refs = reachability_graph
        .data_references_in_data_section
        .remove(&current);
...
fn add_to_optimize_queue(
    ...
    for &next in all_blocks[block_target.index()]
        .next
        .instruction
        .targets()
        .into_iter()
...
fn perform_inlining(
    ...
    fn inline(
        ...
        log::trace!(
            "  {outer:?} will now end with: {:?}",
            all_blocks[inner.index()].next.instruction
        );
        ...
        assert_eq!(
            all_blocks[outer.index()].next.instruction,
            ControlInst::Jump { target: inner }
        );
        ...
        all_blocks[outer.index()]
            .next
            .source
            .overlay_on_top_of_inplace(&inner_source);
    ...
    fn should_inline(
        ...
        if let Some(fallthrough_target) = all_blocks[target.index()]
            .next
            .instruction
            .fallthrough_target()
        {
    ...
    match block.next.instruction {
        ControlInst::Jump { target } => {
            if all_blocks[current.index()].ops.is_empty()
                && inline_history.insert((current, target))
            {
...
fn update_references(
    ...
    for ext in old_references.into_iter().chain(new_references.into_iter()) {
        match ext {
            ...
            ExtRef::DataAddress(target) => {
                remove_if_data_is_globally_unreachable(
                    all_blocks,
                    reachability_graph,
                    optimize_queue.as_deref_mut(),
                    target,
...
fn perform_dead_code_elimination(
    ...
    fn perform_dead_code_elimination_on_block(
        ...
        for (nth_instruction, (_, op)) in all_blocks[block_target.index()]
            .ops
            .iter()
            .enumerate()
            .rev()
    ...
    for &previous_block in &reachability_graph
        .for_code
        .get(&block_target)
        .unwrap()
        .reachable_from
    {
        ...
        let ControlInst::Jump { target } = all_blocks[previous_block.index()].next.instruction
        else {
            continue;
        };
    ...
    let registers_needed_for_next_block = match all_blocks[block_target.index()].next.instruction {
        ...
        ControlInst::Branch {
            ...
            registers_needed_for_block[target_true.index()]
                | registers_needed_for_block[target_false.index()]
        }
...
impl RegValue {
    ...
    fn bits_used(self) -> u64 {
        match self {
            ...
            RegValue::Unknown { bits_used, .. }
            | RegValue::InputReg { bits_used, .. }
            | RegValue::OutputReg { bits_used, .. } => bits_used,
        }
    }
}
...
impl BlockRegs {
    ...
    fn simplify_instruction<H>(
        &self,
        elf: &Elf<H>,
        instruction: BasicInst<AnyTarget>,
    ) -> Option<BasicInst<AnyTarget>>
    where
        H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
    {
            ...
            BasicInst::LoadIndirect {
                ...
                if let RegValue::DataAddress(base) = self.get_reg(base) {
                    return Some(BasicInst::LoadAbsolute {
                        kind,
                        dst,
                        target: base.map_offset_i64(|base| {
                            base.wrapping_add(cast(offset).to_i64_sign_extend())
                        }),
                    });
                }
            }
    ...
    fn set_reg_from_instruction(
        ...
        match instruction {
            ...
            BasicInst::AnyAny {
                ...
                let src1_value = self.get_reg(src1);
                let src2_value = self.get_reg(src2);
                let bits_used = src1_value.bits_used()
                    | src2_value.bits_used()
                    | (src1_value.bits_used() << 1)
                    | (src2_value.bits_used() << 1);
    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    ...
    for nth_instruction in 0..all_blocks[current.index()].ops.len() {
        ...
        while let Some(new_instruction) = regs.simplify_instruction(elf, instruction) {
            log::trace!(
                "Simplifying instruction in {}",
                all_blocks[current.index()].ops[nth_instruction].0
            );
    ...
    if let Some((extra_instruction, new_instruction)) =
        regs.simplify_control_instruction(elf, all_blocks[current.index()].next.instruction)
    {
    ...
    if let Some(ref mut optimize_queue) = optimize_queue {
        if output_regs_modified {
            match all_blocks[current.index()].next.instruction {
                ControlInst::Jump { target } => {
                    add_to_optimize_queue(all_blocks, reachability_graph, optimize_queue, target)
                }
                ControlInst::Branch {
                    target_true,
                    target_false,
...
fn perform_load_address_and_jump_fusion(
    all_blocks: &mut [BasicBlock<AnyTarget, BlockTarget>],
    reachability_graph: &ReachabilityGraph,
) {
    ...
    for window in used_blocks.windows(2) {
        ...
        all_blocks[current.index()].next.instruction =
            match all_blocks[current.index()].next.instruction {
                ...
                ControlInst::JumpIndirect { base, offset } if dst != base => {
                    ControlInst::CallIndirect {
                        base,
                        offset,
                        target_return,
...
mod test {
    ...
    impl ProgramBuilder {
        ...
        fn push(
            &mut self,
            inst: impl Into<InstExt<SectionTarget, SectionTarget>>,
        ) -> SectionTarget {
        ...
        fn append_assembly(&mut self, assembly: &str) {
            ...
            for instruction in blob.instructions(Bitness::B32) {
                ...
                program_counter_to_instruction_index
                    .insert(instruction.offset, self.instructions.len());
            ...
            for instruction in blob.instructions(Bitness::B32) {
                let out = &mut self.instructions[*program_counter_to_instruction_index
                    .get(&instruction.offset)
                    .unwrap()]
                .1;
                match instruction.kind {
                    Instruction::fallthrough => {
                        let target = *program_counter_to_section_target
                            .get(&instruction.next_offset)
                            .unwrap();
                        *out = ControlInst::Jump { target }.into();
                    }
                    Instruction::jump(target) => {
                        let target = *program_counter_to_section_target
                            .get(&polkavm::ProgramCounter(target))
                            .unwrap();
                        *out = ControlInst::Jump { target }.into();
                    }
                    ...
                    | Instruction::branch_eq_imm(src1, src2, target) => {
                        let target_true = *program_counter_to_section_target
                            .get(&polkavm::ProgramCounter(target))
                            .unwrap();
                        let target_false = *program_counter_to_section_target
                            .get(&instruction.next_offset)
                            .unwrap();
                        *out = ControlInst::Branch {
                            kind: match instruction.kind {
                                Instruction::branch_less_unsigned_imm(..) => {
                                    BranchKind::LessUnsigned32
                                }
        ...
        fn build(&self, config: Config) -> TestProgram {
            ...
            let code_sections_set: HashSet<_> =
                self.next_offset_for_section.keys().copied().collect();
            ...
            let all_blocks = split_code_into_basic_blocks(
                &elf,
                &Default::default(),
                &all_jump_targets,
            ...
            let mut all_blocks =
                resolve_basic_block_references(&data_sections_set, &section_to_block, &all_blocks)
                    .unwrap();
            let mut reachability_graph = calculate_reachability(
                &section_to_block,
                &all_blocks,
                &data_sections_set,
                &exports,
            ...
            if matches!(config.opt_level, OptLevel::O2) {
                used_blocks = add_missing_fallthrough_blocks(
                    &mut all_blocks,
                    &mut reachability_graph,
                    used_blocks,
                );
                merge_consecutive_fallthrough_blocks(
                    &mut all_blocks,
                    &mut reachability_graph,
                    &mut section_to_block,
                    &mut used_blocks,
                );
                replace_immediates_with_registers(&mut all_blocks, &imports, &used_blocks);
            }

            let expected_reachability_graph = calculate_reachability(
                &section_to_block,
                &all_blocks,
                &data_sections_set,
                &exports,
            ...
            let (jump_table, jump_target_for_block) =
                build_jump_table(all_blocks.len(), &used_blocks, &reachability_graph);
            let code = emit_code(
                &Default::default(),
                &imports,
                &base_address_for_section,
            ...
            for current in used_blocks {
                for &export_index in &reachability_graph.for_code.get(&current).unwrap().exports {
                    ...
                    builder.add_export_by_basic_block(
                        jump_target.static_target,
                        &export.metadata.symbol,
                    );
                    export_count += 1;
                }
            }
            ...
            let mut disassembler = polkavm_disassembler::Disassembler::new(
                &blob,
    ...
    fn expect_regs(
        regs: impl IntoIterator<Item = (Reg, u64)> + Clone,
    ) -> impl FnMut(&mut polkavm::RawInstance, &mut polkavm::RawInstance) {
...
fn add_missing_fallthrough_blocks(
    ...
    for current in used_blocks {
        ...
        let Some(target) = all_blocks[current.index()]
            .next
            .instruction
            .fallthrough_target_mut()
            .copied()
        else {
            continue;
        };
...
fn spill_fake_registers(
    ...
    impl<'a> regalloc2::Function for RegAllocBlock<'a> {
        ...
        fn block_insns(&self, _block: regalloc2::Block) -> regalloc2::InstRange {
            regalloc2::InstRange::forward(
                regalloc2::Inst(0),
                regalloc2::Inst(self.instructions.len() as u32),
            )
        }
    ...
    for current in used_blocks {
        let block = &mut all_blocks[current.index()];
        let Some(start_at) = block.ops.iter().position(|(_, instruction)| {
            !((instruction.src_mask(imports) | instruction.dst_mask(imports)) & fake_mask)
                .is_empty()
        }) else {
            continue;
        };
        ...
        let end_at = {
            ...
            for index in start_at..block.ops.len() {
                ...
                if !((instruction.src_mask(imports) | instruction.dst_mask(imports)) & fake_mask)
                    .is_empty()
                {
                    end_at = index + 1;
                }
        ...
        for reg in RegMask::all() {
            ...
            prologue.push(regalloc2::Operand::new(
                ...
                regalloc2::OperandConstraint::FixedReg(regalloc2::PReg::new(
                    reg as usize,
        ...
        for nth_instruction in start_at..=end_at {
            while let Some((next_edit_at, edit)) = edits.peek() {
                let target_nth_instruction: isize =
                    next_edit_at.inst().index() as isize - 1 + start_at as isize;
                if target_nth_instruction < 0
                    || target_nth_instruction > nth_instruction as isize
                    || (target_nth_instruction == nth_instruction as isize
                        && next_edit_at.pos() == regalloc2::InstPosition::After)
                {
                    break;
                }
            ...
            let new_instruction = instruction
                .map_register(|reg, _| {
                    ...
                    let allocated_reg =
                        Reg::from_usize(alloc.as_reg().unwrap().hw_enc() as usize).unwrap();
                    if reg.fake_register_index().is_none() {
    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
        ...
        match instruction {
                ...
                if let Some(target) = code_or_data_target {
                    if code_sections_set.contains(&target.section_index) {
                        if all_jump_targets.insert(target) {
                            log::trace!(
                                "Adding jump target: {target} (referenced indirectly by code)"
                            );
                        }
            ...
            InstExt::Control(instruction) => {
                for target in instruction.targets().into_iter().flatten() {
                    ...
                    if all_jump_targets.insert(*target) {
                        log::trace!(
                            "Adding jump target: {target} (referenced by a control instruction)"
                        );
                    }
                }
            }
    ...
    for export in exports {
        ...
        if !code_sections_set.contains(&target.section_index) {
            return Err(ProgramFromElfError::other(
                "export points to a non-code section",
            ));
        }
...
fn each_reference_for_basic_instruction(
    instruction: &BasicInst<AnyTarget>,
    mut cb: impl FnMut(ExtRef),
) {
    let (data_target, code_or_data_target) = instruction.target();
    if let Some(target) = data_target {
...
fn each_reference_for_control_instruction(
    instruction: &ControlInst<BlockTarget>,
    mut cb: impl FnMut(ExtRef),
) {
    match *instruction {
...
fn build_jump_table(
    ...
    for (static_target, current) in used_blocks.iter().enumerate() {
        ...
        let dynamic_target = if reachability.is_dynamically_reachable() {
            let dynamic_target: u32 = (jump_table.len() + 1)
                .try_into()
...
fn emit_code(
    ...
    let can_fallthrough_to_next_block = calculate_whether_can_fallthrough(all_blocks, used_blocks);
    let get_data_address =
        |source: &SourceStack, target: SectionTarget| -> Result<u32, ProgramFromElfError> {
            if let Some(&base_address) = base_address_for_section.get(&target.section_index) {
                ...
                Err(ProgramFromElfError::other(
                    "internal error: section with no base address",
                ))
            }
        };
    ...
    for block_target in used_blocks {
        ...
        if !basic_block_delimited {
            ...
            code.push((
                Source {
                    ...
                    offset_range: (block.source.offset_range.start
                        ..block.source.offset_range.start + 4)
                        .into(),
                }
                .into(),
                Instruction::fallthrough,
        ...
        for (source, op) in &block.ops {
            let op = match *op {
                BasicInst::LoadImmediate { dst, imm } => {
                    Instruction::load_imm(conv_reg(dst), cast(imm).to_unsigned())
                }
                BasicInst::LoadImmediate64 { dst, imm } => {
                    if !is_rv64 {
                        unreachable!(
                            "internal error: load_imm64 found when processing 32-bit binary"
                        )
                    } else {
                    ...
                    let value = match target {
                        AnyTarget::Code(target) => {
                            ...
                            let Some(value) = value.checked_mul(VM_CODE_ADDRESS_ALIGNMENT) else {
                                return Err(ProgramFromElfError::other(
                                    "overflow when emitting an address load",
                                ));
                            };
                            value
                        }
                ...
                BasicInst::MoveReg { dst, src } => {
                    Instruction::move_reg(conv_reg(dst), conv_reg(src))
                }
                BasicInst::AnyAny {
                    kind,
                    dst,
                    ...
                    match (src1, src2) {
                        ...
                        (RegImm::Reg(src1), RegImm::Imm(src2)) => {
                            ...
                            match kind {
                                ...
                                K::Sub32 => I::add_imm_32(
                                    dst,
                                    src1,
                                    cast(-cast(src2).to_signed()).to_unsigned(),
                                ),
                                ...
                                K::ShiftLogicalLeft32 => {
                                    I::shift_logical_left_imm_32(dst, src1, src2)
                                }
                                K::ShiftLogicalLeft32AndSignExtend => {
                                    I::shift_logical_left_imm_32(dst, src1, src2)
                                }
                                K::ShiftLogicalLeft64 => {
                                    I::shift_logical_left_imm_64(dst, src1, src2)
                                }
                                K::SetLessThanSigned32 => {
                                    I::set_less_than_signed_imm(dst, src1, src2)
                                }
                                K::SetLessThanSigned64 => {
                                    I::set_less_than_signed_imm(dst, src1, src2)
                                }
                                K::SetLessThanUnsigned32 => {
                                    I::set_less_than_unsigned_imm(dst, src1, src2)
                                }
                                K::SetLessThanUnsigned64 => {
                                    I::set_less_than_unsigned_imm(dst, src1, src2)
                                }
                                K::Xor32 | K::Xor64 => I::xor_imm(dst, src1, src2),
                                K::ShiftLogicalRight32 => {
                                    I::shift_logical_right_imm_32(dst, src1, src2)
                                }
                                K::ShiftLogicalRight32AndSignExtend => {
                                    I::shift_logical_right_imm_32(dst, src1, src2)
                                }
                                K::ShiftLogicalRight64 => {
                                    I::shift_logical_right_imm_64(dst, src1, src2)
                                }
                                K::ShiftArithmeticRight32 => {
                                    I::shift_arithmetic_right_imm_32(dst, src1, src2)
                                }
                                K::ShiftArithmeticRight32AndSignExtend => {
                                    I::shift_arithmetic_right_imm_32(dst, src1, src2)
                                }
                                K::ShiftArithmeticRight64 => {
                                    I::shift_arithmetic_right_imm_64(dst, src1, src2)
                                }
                                ...
                                K::RotateRight32AndSignExtend => {
                                    I::rotate_right_imm_32(dst, src1, src2)
                                }
                        ...
                        (RegImm::Imm(src1), RegImm::Reg(src2)) => {
                            ...
                            match kind {
                                ...
                                K::ShiftLogicalLeft32 => {
                                    I::shift_logical_left_imm_alt_32(dst, src2, src1)
                                }
                                K::ShiftLogicalLeft32AndSignExtend => {
                                    I::shift_logical_left_imm_alt_32(dst, src2, src1)
                                }
                                K::ShiftLogicalLeft64 => {
                                    I::shift_logical_left_imm_alt_64(dst, src2, src1)
                                }
                                K::SetLessThanSigned32 => {
                                    I::set_greater_than_signed_imm(dst, src2, src1)
                                }
                                K::SetLessThanSigned64 => {
                                    I::set_greater_than_signed_imm(dst, src2, src1)
                                }
                                K::SetLessThanUnsigned32 => {
                                    I::set_greater_than_unsigned_imm(dst, src2, src1)
                                }
                                K::SetLessThanUnsigned64 => {
                                    I::set_greater_than_unsigned_imm(dst, src2, src1)
                                }
                                K::ShiftLogicalRight32 => {
                                    I::shift_logical_right_imm_alt_32(dst, src2, src1)
                                }
                                K::ShiftLogicalRight32AndSignExtend => {
                                    I::shift_logical_right_imm_alt_32(dst, src2, src1)
                                }
                                K::ShiftLogicalRight64 => {
                                    I::shift_logical_right_imm_alt_64(dst, src2, src1)
                                }
                                K::ShiftArithmeticRight32 => {
                                    I::shift_arithmetic_right_imm_alt_32(dst, src2, src1)
                                }
                                K::ShiftArithmeticRight32AndSignExtend => {
                                    I::shift_arithmetic_right_imm_alt_32(dst, src2, src1)
                                }
                                K::ShiftArithmeticRight64 => {
                                    I::shift_arithmetic_right_imm_alt_64(dst, src2, src1)
                                }

                                K::RotateRight32 => I::rotate_right_imm_alt_32(dst, src2, src1),
                                K::RotateRight32AndSignExtend => {
                                    I::rotate_right_imm_alt_32(dst, src2, src1)
                                }
                        ...
                        (RegImm::Imm(src1), RegImm::Imm(src2)) => {
                            if is_optimized {
                                ...
                                let imm: u32 = OperationKind::from(kind)
                                    .apply_const(
                                        cast(src1).to_i64_sign_extend(),
                                        cast(src2).to_i64_sign_extend(),
                                    )
                                    .try_into()
                                    .expect("load immediate overflow");
                                I::load_imm(dst, imm)
                ...
                BasicInst::Ecalli { nth_import } => {
                    ...
                    Instruction::ecalli(
                        import
                            .metadata
                            .index
                            .expect("internal error: no index was assigned to an ecall"),
                    )
                }
        ...
        match block.next.instruction {
            ...
            ControlInst::Call {
                ...
                let target_return = get_jump_target(target_return)?
                    .dynamic_target
                    .expect("missing jump target for address");
                let Some(target_return) = target_return.checked_mul(VM_CODE_ADDRESS_ALIGNMENT)
                else {
                    return Err(ProgramFromElfError::other(
                        "overflow when emitting an indirect call",
                    ));
                };

                code.push((
                    block.next.source.clone(),
                    Instruction::load_imm_and_jump(
                        conv_reg(ra),
                        target_return,
            ...
            ControlInst::JumpIndirect { base, offset } => {
                ...
                code.push((
                    block.next.source.clone(),
                    Instruction::jump_indirect(conv_reg(base), offset),
                ));
            }
            ControlInst::CallIndirect {
                ra,
                ...
                code.push((
                    block.next.source.clone(),
                    Instruction::load_imm_and_jump_indirect(
                        conv_reg(ra),
                        conv_reg(base),
                        target_return,
                        offset,
                ...
                let instruction = match (src1, src2) {
                    ...
                    (RegImm::Imm(src1), RegImm::Imm(src2)) => {
                        if is_optimized {
                            ...
                            match OperationKind::from(kind).apply_const(
                                cast(src1).to_i64_sign_extend(),
                                cast(src2).to_i64_sign_extend(),
                            ) {
    ...
    fn targets(&self) -> [Option<SectionTarget>; 2] {
        match self {
            ...
            RelocationKind::JumpTable {
                target_code,
                target_base,
            } => [Some(*target_code), Some(*target_base)],
        }
    }
}
    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    ...
    for (absolute_address, relocation) in section.relocations() {
        let Some(relative_address) = absolute_address.checked_sub(section.original_address())
        else {
        ...
        let (relocation_name, kind) = match (relocation.kind(), relocation.flags()) {
            (object::RelocationKind::Absolute, _)
                if relocation.encoding() == object::RelocationEncoding::Generic
                    && relocation.size() == 32 =>
            {
                (
                    "R_RISCV_32",
            ...
            (_, object::RelocationFlags::Elf { r_type: reloc_kind }) => match reloc_kind {
                ...
                object::elf::R_RISCV_ADD8 => (
                    "R_RISCV_ADD8",
                    Kind::Mut(MutOp::Add, RelocationSize::U8, target),
                ),
                object::elf::R_RISCV_SUB8 => (
                    "R_RISCV_SUB8",
                    Kind::Mut(MutOp::Sub, RelocationSize::U8, target),
                ),
                object::elf::R_RISCV_ADD16 => (
                    "R_RISCV_ADD16",
                    Kind::Mut(MutOp::Add, RelocationSize::U16, target),
                ),
                object::elf::R_RISCV_SUB16 => (
                    "R_RISCV_SUB16",
                    Kind::Mut(MutOp::Sub, RelocationSize::U16, target),
                ),
                object::elf::R_RISCV_ADD32 => (
                    "R_RISCV_ADD32",
                    Kind::Mut(MutOp::Add, RelocationSize::U32, target),
                ),
                object::elf::R_RISCV_ADD64 => (
                    "R_RISCV_ADD64",
                    Kind::Mut(MutOp::Add, RelocationSize::U64, target),
                ),
                object::elf::R_RISCV_SUB32 => (
                    "R_RISCV_SUB32",
                    Kind::Mut(MutOp::Sub, RelocationSize::U32, target),
                ),
                object::elf::R_RISCV_SUB64 => (
                    "R_RISCV_SUB64",
                    Kind::Mut(MutOp::Sub, RelocationSize::U64, target),
                ),
                object::elf::R_RISCV_SET_ULEB128 => {
                    ("R_RISCV_SET_ULEB128", Kind::SetUleb128 { target })
                }
                object::elf::R_RISCV_SUB_ULEB128 => {
                    ("R_RISCV_SUB_ULEB128", Kind::SubUleb128 { target })
                }
...
fn overwrite_uleb128(
    ...
    loop {
        let Some(byte) = data.get_mut(data_offset) else {
            return Err(ProgramFromElfError::other(
                "ULEB128 relocation target offset out of bounds",
            ));
        };
        data_offset += 1;
    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    fn jump_or_call<T>(
        ...
        target: T,
        target_return: T,
    ) -> Result<ControlInst<T>, ProgramFromElfError> {
            ...
            (_, object::RelocationFlags::Elf { r_type: reloc_kind }) => {
                ...
                match reloc_kind {
                    object::elf::R_RISCV_CALL_PLT => {
                        ...
                        let Some(xs) = section_data.get(
                            current_location.offset as usize..current_location.offset as usize + 8,
                        ) else {
                            return Err(ProgramFromElfError::other(
                                "invalid R_RISCV_CALL_PLT relocation",
                            ));
                        };
                        ...
                        let Inst::AddUpperImmediateToPc {
                            dst: hi_reg,
                            value: _,
                        } = hi_inst
                        else {
                    ...
                    object::elf::R_RISCV_JAL => {
                        ...
                        instruction_overrides.insert(
                            current_location,
                            InstExt::Control(jump_or_call(dst, target, target_return)?),
                        );
                    ...
                    object::elf::R_RISCV_HI20 => {
                        ...
                        let Some(dst) = cast_reg_non_zero(dst)? else {
                            return Err(ProgramFromElfError::other(
                                "R_RISCV_HI20 with a zero destination register",
                            ));
                        };
                    ...
                    object::elf::R_RISCV_RVC_JUMP => {
                        ...
                        let (Inst::JumpAndLink { dst, .. } | Inst::JumpAndLinkRegister { dst, .. }) =
                            inst
                        else {
                            return Err(ProgramFromElfError::other(format!(
    ...
    for (relative_lo, (lo_rel_name, relative_hi)) in pcrel_relocations.reloc_pcrel_lo12 {
        ...
        let lo_inst_raw = u32::from_le_bytes([
            lo_inst_raw[0],
            lo_inst_raw[1],
            lo_inst_raw[2],
            lo_inst_raw[3],
        ...
        let hi_inst_raw = u32::from_le_bytes([
            hi_inst_raw[0],
            hi_inst_raw[1],
            hi_inst_raw[2],
            hi_inst_raw[3],
        ...
        let Some((hi_kind, target)) = pcrel_relocations
            .reloc_pcrel_hi20
            .get(&relative_hi)
            .copied()
        else {
        ...
        let (lo_reg, new_instruction) = if matches!(hi_kind, HiRelocKind::Got) {
            ...
            match lo_inst {
                Inst::Load {
                    ...
                    (
                        base,
                        InstExt::Basic(BasicInst::LoadAddressIndirect { dst, target }),
                    )
                }
                Inst::Load {
                    kind: LoadKind::I32,
...
pub fn program_from_elf(config: Config, data: &[u8]) -> Result<Vec<u8>, ProgramFromElfError> {
    match Elf::<object::elf::FileHeader32<object::endian::LittleEndian>>::parse(data) {
        ...
        Err(ProgramFromElfError(ProgramFromElfErrorKind::FailedToParseElf(e)))
            if e.to_string() == "Unsupported ELF header" =>
        {
...
fn program_from_elf_internal<H>(
    config: Config,
    mut elf: Elf<H>,
) -> Result<Vec<u8>, ProgramFromElfError>
where
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    let is_rv64 = elf.is_64();
    ...
    for section in elf.sections() {
            ...
            || name.starts_with(".sbss.")
        {
            ...
            || (section.is_allocated() && section.is_executable())
        {
            if is_writable {
                return Err(ProgramFromElfError::other(format!(
                    "expected section '{name}' to be read-only, yet it is writable"
    ...
    for &section_index in &sections_code {
        ...
        if instructions.len() > initial_instruction_count {
            ...
            let source = Source {
                ...
                offset_range: (last_source.offset_range.end..last_source.offset_range.end + 4)
                    .into(),
            };
    ...
    if !instruction_overrides.is_empty() {
        return Err(ProgramFromElfError::other(
            "internal error: instruction overrides map is not empty",
        ));
    }
    ...
    {
        let strip_relocations_for_sections: HashSet<_> = sections_metadata
            .iter()
            .copied()
            .chain(sections_exports.iter().copied())
            .collect();

        relocations.retain(|relocation_target, _| {
            !strip_relocations_for_sections.contains(&relocation_target.section_index)
        });
    }
    ...
    if matches!(config.opt_level, OptLevel::O1 | OptLevel::O2) {
        reachability_graph = calculate_reachability(
            &section_to_block,
            &all_blocks,
            &data_sections_set,
            &exports,
        ...
        used_blocks =
            add_missing_fallthrough_blocks(&mut all_blocks, &mut reachability_graph, used_blocks);
        merge_consecutive_fallthrough_blocks(
            &mut all_blocks,
            &mut reachability_graph,
            &mut section_to_block,
        ...
        if reachability_graph != expected_reachability_graph {
            ...
            {
                let _ = std::fs::write(
                    "/tmp/reachability_graph_actual.txt",
                    format!("{reachability_graph:#?}"),
                );
                let _ = std::fs::write(
                    "/tmp/reachability_graph_expected.txt",
                    format!("{expected_reachability_graph:#?}"),
                );
            }
        ...
        for &section_index in sections_ro_data.iter().chain(sections_rw_data.iter()) {
            let reachability = reachability_graph
                .for_data
                .entry(section_index)
                .or_default();
...

    {
        ...
        for &section_index in &sections_other {
            ...
            assert!(base_address_for_section
                .insert(section_index, address)
                .is_none());
        }
    }
        ...
        match relocation {
            RelocationKind::Offset {
                ...
                if reachability_graph.is_data_section_reachable(section.index())
                    && !matches!(size, SizeRelocationSize::Generic(..))
                {
                    return Err(ProgramFromElfError::other(format!(
                ...
                let Some(&origin_section_address) =
                    base_address_for_section.get(&origin.section_index)
                else {
                    return Err(ProgramFromElfError::other(format!(
                ...
                let Some(&target_section_address) =
                    base_address_for_section.get(&target.section_index)
                else {
                    return Err(ProgramFromElfError::other(format!(
                ...
                let range = origin_section_address.wrapping_add(origin.offset)
                    ..target_section_address.wrapping_add(target.offset);
                ...
                match size {
                    ...
                    SizeRelocationSize::SixBits => {
                        ...
                        let output = (u64::from(read_u8(data, relocation_target.offset)?)
                            & (!mask))
                            | (value & mask);
                ...
                if let Some(&block_target) = section_to_block.get(&target) {
                    let Some(jump_target) = jump_target_for_block[block_target.index()] else {
                        if !reachability_graph
                            .is_data_section_reachable(relocation_target.section_index)
                        {
                    ...
                    let jump_target = jump_target
                        .dynamic_target
                        .expect("missing jump target for address");
                    let Some(jump_target) = jump_target.checked_mul(VM_CODE_ADDRESS_ALIGNMENT)
                    else {
                        return Err(ProgramFromElfError::other(
                            "overflow when applying a jump target relocation",
                        ));
                    };
                    ...
                    let Some(section_base) = base_address_for_section.get(&target.section_index)
                    else {
                        if !reachability_graph
                            .is_data_section_reachable(relocation_target.section_index)
                        {
                    ...
                    let Some(value) = section_base.checked_add(target.offset) else {
                        return Err(ProgramFromElfError::other(
                            "overflow when applying an absolute relocation",
                        ));
                    };
            ...
            RelocationKind::JumpTable {
                ...
                let Some(section_base) = base_address_for_section.get(&target_base.section_index)
                else {
                    return Err(ProgramFromElfError::other(
    ...
    if !config.strip {
        ...
        let dwarf_info =
            crate::dwarf::load_dwarf(&mut string_cache, &elf, &relocations, &section_map)?;
    ...
    for current in used_blocks {
        for &export_index in &reachability_graph.for_code.get(&current).unwrap().exports {
            let export = &exports[export_index];
            let jump_target = jump_target_for_block[current.index()].expect(
                "internal error: export metadata points to a block without a jump target assigned",
            );
    ...
    let mut locations_for_instruction: Vec<Option<Arc<[Location]>>> =
        ...
        'outer: for (nth_instruction, locations) in locations_for_instruction.iter().enumerate() {
            ...
            while let Some(region_info) = line_program.run().unwrap() {
                ...
                for (actual, expected) in region_info.frames().zip(locations.iter()) {
                    ...
                    assert_eq!(
                        actual.function_name_without_namespace().unwrap(),
                        expected.function_name.as_deref()
                    );
                    assert_eq!(
...
fn simplify_path(path: &str) -> Cow<'_, str> {
...
fn emit_debug_info(
    ...
    impl<'a> DebugStringsBuilder<'a> {
        fn dedup_cow(&mut self, s: Cow<'a, str>) -> u32 {
            ...
            let length = varint::write_varint(
                s.len().try_into().expect("debug string length overflow"),
                &mut buffer,
            );
            ...
            let offset: u32 = offset
                .try_into()
                .expect("debug string section length overflow");
    ...
    for (instruction_position, locations) in locations_for_instruction.iter().enumerate() {
        let group = if let Some(locations) = locations {
            ...
            Group {
                ...
                path: location
                    .source_code_location
                    .as_ref()
                    .map(|target| simplify_path(target.path())),
        ...
        if let Some(last_group) = groups.last_mut() {
            if last_group.key() == group.key() {
                assert_eq!(
                    last_group.instruction_position + last_group.instruction_count,
                    instruction_position
                );
    ...
    let mut info_offsets = Vec::with_capacity(groups.len());
    {
        ...
        for group in &groups {
            let info_offset: u32 = (writer.len() - offset_base)
                .try_into()
                ...
                fn finish_instruction(
                    ...
                    match (self.queued_count == 1, dir) {
                        (true, Direction::GoDown) => {
                            writer.push_byte(
                                LineProgramOp::FinishInstructionAndIncrementStackDepth as u8,
                            );
                        }
                        ...
                        (true, Direction::GoUp) => {
                            writer.push_byte(
                                LineProgramOp::FinishInstructionAndDecrementStackDepth as u8,
                            );
                        }
            ...
            for nth_instruction in
                group.instruction_position..group.instruction_position + group.instruction_count
            {
                ...
                for (depth, location) in locations.iter().enumerate() {
                    ...
                    let changed_function_name =
                        state.stack[depth].function_name != location.function_name;
                    let changed_path =
                        state.stack[depth].path.as_deref().map(Cow::Borrowed) != new_path;
                    ...
                    if changed_path {
                        ...
                        state.stack[depth].path =
                            location.source_code_location.as_ref().map(|location| {
                                match simplify_path(location.path()) {
                        ...
                        let path_offset = location
                            .source_code_location
                            .as_ref()
                            .map_or(empty_string_id, |location| {
                                dbg_strings.dedup_cow(simplify_path(location.path()))
                            });
                ...
                state.finish_instruction(
                    writer,
                    next_depth,
                    (offsets[nth_instruction].1).0 - (offsets[nth_instruction].0).0,
                );
            }
    ...
    builder.add_custom_section(
        ...
        section_line_programs,
    );
    builder.add_custom_section(
        program::SECTION_OPT_DEBUG_LINE_PROGRAM_RANGES,
        section_line_program_ranges,
    );
}
