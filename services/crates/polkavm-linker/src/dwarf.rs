...
use crate::program_from_elf::{
    AddressRange, RelocationKind, RelocationSize, SectionTarget, SizeRelocationSize, Source,
};
...
{
    let target_unit =
        units.binary_search_by_key(&target_offset.0, |target_unit| target_unit.offset.0);
    ...
    R: gimli::Reader,
{
    if unit.encoding().version <= 4 {
        ...
        if let Some((start_section, start_range)) =
            try_fetch_size_relocation(relocations, relocation_start, is_64bit)?
        {
            let (end_section, end_range) =
                fetch_size_relocation(relocations, relocation_end, is_64bit)?;
        ...
        loop {
            ...
            match kind {
                ...
                gimli::constants::DW_RLE_offset_pair => {
                    ...
                    if let Some(base) = base {
                        let source = Source {
                            ...
                            offset_range: (base.offset + offset_start..base.offset + offset_end)
                                .into(),
                        };
                ...
                gimli::constants::DW_RLE_startx_length => {
                    let begin =
                        gimli::DebugAddrIndex(reader.read_uleb128().and_then(R::Offset::from_u64)?);
                    let length = reader.read_uleb128()?;
                    if let Some(target) = resolve_debug_addr_index(
                        sections.debug_addr,
                        relocations,
                        unit,
                        begin,
...
impl<R: gimli::Reader> AttributeParser<R> {
    ...
    fn try_match_attribute(
        ...
        match name {
            gimli::DW_AT_low_pc => match value.clone() {
                AttributeValue {
                    ...
                    if let Some(target) =
                        try_fetch_relocation(relocations, relocation_target, self.is_64bit)?
                    {
                ...
                AttributeValue {
                    ...
                    self.low_pc = resolve_debug_addr_index(
                        sections.debug_addr,
                        relocations,
                        &unit.raw_unit,
                        index,
            ...
            gimli::DW_AT_high_pc => match value {
                ...
                AttributeValue {
                    ...
                    self.high_pc = resolve_debug_addr_index(
                        sections.debug_addr,
                        relocations,
                        &unit.raw_unit,
                        index,
            ...
            gimli::DW_AT_ranges => match value {
                ...
                AttributeValue {
                    ...
                    self.ranges_offset = Some(dwarf.ranges_offset_from_raw(
                        &unit.raw_unit,
                        gimli::RawRangeListsOffset(offset),
                    ));
                    Ok(())
                }
                _ => Err(UnsupportedValue(value)),
                ...
                match value {
                    AttributeValue {
                        ...
                        self.abstract_origin =
                            Some(offset.to_debug_info_offset(&unit.raw_unit.header).unwrap());
                        Ok(())
                    }
                    AttributeValue {
        ...
        .map_err(move |UnsupportedValue(value): UnsupportedValue<R>| {
            ProgramFromElfError::other(format!(
                "failed to process DWARF: unsupported value for {name}: {value:?}"
            ))
        })
    }
    ...
    fn resolve_while(
        ...
        if callback(unit, self)? {
            if let Some((target_unit, parser)) =
                self.resolve_abstract_origin(sections, relocations, dwarf, units)?
            {
                return parser.resolve_while(
                    sections,
                    relocations,
                    dwarf,
                    units,
...
fn try_fetch_size_relocation(
    ...
    match relocation {
        ...
        } if origin.section_index == target.section_index && target.offset >= origin.offset => Ok(
            Some((origin.section_index, (origin.offset..target.offset).into())),
        ),
        RelocationKind::Abs {
            target,
            size: RelocationSize::U32,
            ...
            (target.offset..target.offset).into(),
        ))),
        RelocationKind::Abs {
            target,
            size: RelocationSize::U64,
    ...
    R: gimli::Reader,
{
    if let Some(section) = debug_addr {
        let address = unit.addr_base.0
            + R::Offset::from_u64(index.0.into_u64() * u64::from(unit.encoding().address_size))?;
        let offset = address
            .into_u64()
        ...
        Err(ProgramFromElfError::other(
            "failed to process DWARF: missing '.debug_addr' section",
        ))
    }
}
    ...
    R: gimli::Reader,
{
    ...
    for dir in header.include_directories() {
        let value = dwarf
            .attr_string(raw_unit, dir.clone())?
            .to_string_lossy()?
            .into_owned();
        dirs.push(value);
    }
    ...
    for file in header.file_names().iter() {
        let filename = dwarf
            .attr_string(raw_unit, file.path_name())?
            .to_string_lossy()?
            .into_owned();
        ...
        let separator = if has_windows_root(&filename)
            || has_windows_root(directory)
            || has_windows_root(&compilation_directory)
        {
            '\\'
        } else {
            '/'
        ...
        loop {
            ...
            match instruction {
                ...
                LineInstruction::FixedAddPc(..) => {
                    ...
                    target = try_fetch_size_relocation(relocations, relocation_target, is_64bit)?
                        .map(|(target_section_index, target_range)| SectionTarget {
            ...
            let location = match (row.line(), row.column()) {
                (None, _) => SourceCodeLocation::Path {
                    path: Arc::clone(path),
                },
...
fn get_function_line_region_id(
    ...
    let (path, line) = match location {
        ...
        SourceCodeLocation::Line { path, line } | SourceCodeLocation::Column { path, line, .. } => {
            (path, line)
        }
    };
    ...
    R: gimli::Reader,
{
    fn run(mut self) -> Result<HashMap<SectionTarget, Arc<[Location]>>, ProgramFromElfError> {
        ...
        let mut subprogram_offset_to_namespace: HashMap<
            gimli::DebugInfoOffset<R::Offset>,
            Option<Arc<str>>,
        > = Default::default();
        ...
        for unit in self.units {
            ...
            subprograms.retain(|subprogram| {
                subprogram_offset_to_namespace
                    .insert(subprogram.offset, subprogram.namespace.clone());
                ...
                if !subprogram.is_declaration {
                    ...
                    match subprogram.decl_location {
                        ...
                        Some(
                            SourceCodeLocation::Line { ref path, line }
                            | SourceCodeLocation::Column { ref path, line, .. },
                        ) => {
            ...
            R: gimli::Reader,
        {
            fn eq(&self, rhs: &Self) -> bool {
                match (self, rhs) {
                    ...
                    (Self::Line { entry: lhs, .. }, Self::Line { entry: rhs, .. }) => {
                        core::ptr::eq(*lhs, *rhs)
                    }
                    _ => false,
                }
            ...
            R: gimli::Reader,
        {
            ...
            for offset in
                (inline_source.offset_range.start..inline_source.offset_range.end).step_by(2)
            {
        ...
        for (subprograms, all_lines) in subprograms_for_unit
            .into_iter()
            .zip(lines_for_unit.into_iter())
        {
            ...
            let line_range_map_for_section: HashMap<SectionIndex, RangeMap<&LineEntry>> =
                lines_for_section
                    .into_iter()
                    .map(|(section_index, local_lines)| {
                        let line_boundaries: Vec<u64> = local_lines
                            .iter()
                            .map(|entry| entry.target.offset)
                            .collect();
                        let line_ranges = line_boundaries.windows(2).map(|w| w[0]..w[1]);
                        let line_range_map: RangeMap<&LineEntry> =
                            line_ranges.zip(local_lines.into_iter()).collect();
            ...
            for subprogram in subprograms {
                ...
                let mut last_emitted: Option<(
                    Vec<LocationKindRef<R>>,
                    Arc<[Location]>,
                )> = None;
                for offset in (source.offset_range.start..source.offset_range.end).step_by(2) {
                    ...
                    if let Some(line_entry) = line_range_map.get_value(offset) {
                        let target_position = {
                            ...
                            if let Some(line_entry_region) = get_function_line_region_id(
                                &function_line_boundaries_for_file,
                                &line_entry.location,
                            ) {
                                ...
                                for (position, kind) in list.iter().enumerate() {
                                    match kind {
                                        LocationKindRef::InlineCall(inlined) => {
                                            let call_location =
                                                inlined.call_location.as_ref().unwrap();
                                            if Some(line_entry_region)
                                                == get_function_line_region_id(
                                                    &function_line_boundaries_for_file,
                                                    call_location,
                                                )
                                            {
                                                if line_entry.location < *call_location {
                                                    target_position = Some((
                                                        position,
                                                        inlined.namespace.clone(),
                                                        inlined.function_name.clone(),
                                                    ));
                                                } else {
                                                    target_position = Some((
                                                        position + 1,
                                        ...
                                        LocationKindRef::InlineDecl(inlined) => {
                                            let decl_location =
                                                inlined.decl_location.as_ref().unwrap();
                                            if Some(line_entry_region)
                                                == get_function_line_region_id(
                                                    &function_line_boundaries_for_file,
                                                    decl_location,
                                ...
                                if target_position.is_none() {
                                    if let Some(ref subprogram_decl_location) =
                                        subprogram.decl_location
                                    {
                                        let subprogram_region = get_function_line_region_id(
                                            &function_line_boundaries_for_file,
                                            subprogram_decl_location,
                                        );
                                        if Some(line_entry_region) == subprogram_region {
                                            target_position = Some((
                                                0,
                                                subprogram.namespace.clone(),
                                                subprogram.function_name.clone(),
                                            ));
                                        }
                                    }
                                }
                        ...
                        if !matches!(line_entry.location, SourceCodeLocation::Path { .. }) {
                            if let Some((target_position, namespace, function_name)) =
                                target_position
                            {
                                ...
                                log::warn!(
                                    "No matching DWARF subprogram found for line at {:?}",
                                    line_entry.location
                                );
                                fallback = true;
                    ...
                    log::trace!(
                        ...
                        subprogram.decl_location.as_ref().and_then(|location| {
                            get_function_line_region_id(
                                &function_line_boundaries_for_file,
                                location,
                            )
                    ...
                    for kind in &list {
                        match kind {
                            LocationKindRef::InlineCall(inlined) => {
                                let call_region =
                                    inlined.call_location.as_ref().and_then(|location| {
                                        get_function_line_region_id(
                                            &function_line_boundaries_for_file,
                                            location,
                                        )
                            ...
                            LocationKindRef::InlineDecl(inlined) => {
                                let decl_region =
                                    inlined.decl_location.as_ref().and_then(|location| {
                                        get_function_line_region_id(
                                            &function_line_boundaries_for_file,
                                            location,
                                        )
                            ...
                            LocationKindRef::Line {
                                ...
                                let region = get_function_line_region_id(
                                    &function_line_boundaries_for_file,
                                    &line_entry.location,
                                );
                                log::trace!(
    ...
    fn parse_tree(
        &mut self,
        unit: &Unit<ReaderWrapper<R>>,
    ) -> Result<Vec<SubProgram<R>>, ProgramFromElfError> {
        ...
        self.walk(unit, node)?;
        self.subprograms
            .sort_by_key(|subprogram| subprogram.sources.get(0).copied());
    ...
    fn walk(
        ...
        let Some(node_offset) = node_entry
            .offset()
            .to_debug_info_offset(&unit.raw_unit.header)
        else {
        ...
        match node_tag {
                ...
                parser.resolve_while(
                    ...
                    |parser_unit, parser| {
                        ...
                        if path.is_none() {
                            path = parser
                                .decl_file
                                .and_then(|index| parser_unit.paths.get(index))
                                .cloned();
                        }
            ...
            gimli::DW_TAG_inlined_subroutine => {
                ...
                log::trace!(
                    "  Inlined subroutine decl location: {:?} {:?}",
                    decl_path,
                    decl_line
                );
                log::trace!(
                ...
                log::trace!(
                    "  Inlined subroutine abstract origin: {:?}",
                    parser.abstract_origin
                );
                ...
                let call_location = match (call_path, call_line, call_column) {
                    (Some(path), Some(line), Some(column)) => {
                        Some(SourceCodeLocation::Column { path, line, column })
                    }
        ...
        child_inlined.sort_by_key(|inlined| {
            (
                inlined.source.offset_range.start,
                inlined.depth,
                !inlined.source.offset_range.end,
            )
        });
            ...
            for parent_source in &subprogram.sources {
                ...
                for inlined in &child_inlined {
                    ...
                    if let Some(last_source) = previous {
                        if inline_source
                            .offset_range
                            .is_overlapping(&last_source.offset_range)
                        {
...
{
    use gimli::{
        AttributeValue, DebugAddrIndex, DebugInfoOffset, DebugLineStrOffset, DebugLocListsIndex,
        DebugRngListsIndex, DebugStrOffset, DebugStrOffsetsIndex, DebugTypeSignature, Expression,
        UnitOffset,
    };
    ...
    loop {
        let value = match form {
            ...
            gimli::constants::DW_FORM_data4 => {
                if encoding.format == gimli::Format::Dwarf32
                    && allow_section_offset(attribute.name(), encoding.version)
                {
            ...
            gimli::constants::DW_FORM_ref8 => {
                let reference = input.read_u64().and_then(|offset| {
                    R::Offset::from_u64(offset).map_err(|_| gimli::Error::OffsetOutOfBounds)
                })?;
                AttributeValue::UnitRef(UnitOffset(reference))
            }
            gimli::constants::DW_FORM_ref_udata => {
                let reference = input.read_uleb128().and_then(|offset| {
                    R::Offset::from_u64(offset).map_err(|_| gimli::Error::OffsetOutOfBounds)
                })?;
                AttributeValue::UnitRef(UnitOffset(reference))
            }
            ...
            gimli::constants::DW_FORM_ref_sup8 => {
                let offset = input.read_u64().and_then(|offset| {
                    R::Offset::from_u64(offset).map_err(|_| gimli::Error::OffsetOutOfBounds)
                })?;
                AttributeValue::DebugInfoRefSup(DebugInfoOffset(offset))
            }
            ...
            gimli::constants::DW_FORM_implicit_const => {
                let data = attribute
                    ...
                    .ok_or(gimli::Error::InvalidImplicitConst)?;
                AttributeValue::Sdata(data)
            }
            gimli::constants::DW_FORM_strx | gimli::constants::DW_FORM_GNU_str_index => {
                let index = input.read_uleb128().and_then(|offset| {
                    R::Offset::from_u64(offset).map_err(|_| gimli::Error::OffsetOutOfBounds)
                })?;
            ...
            gimli::constants::DW_FORM_strx3 => {
                let index = input.read_uint(3).and_then(|offset| {
                    R::Offset::from_u64(offset).map_err(|_| gimli::Error::OffsetOutOfBounds)
                })?;
                AttributeValue::DebugStrOffsetsIndex(DebugStrOffsetsIndex(index))
            }
...
fn iter_attributes<'a, R>(
    ...
    impl Iterator<Item = Result<(gimli::constants::DwAt, AttributeValue<R>), ProgramFromElfError>> + 'a,
    ProgramFromElfError,
>
where
    R: gimli::Reader,
{
    ...
    if code != 0 {
        let abbrev = unit
            .abbreviations
            .get(code)
            .ok_or(gimli::Error::UnknownAbbreviation(code))?;
    ...
    let iterator = core::iter::from_fn(move || {
        ...
        Some(
            parse_attribute(&section_base, &mut input, encoding, attribute)
    ...
    for pair in iter_attributes(dwarf, unit, entry_offset)? {
        ...
        match value {
            AttributeValue {
                ...
                return resolve_debug_addr_index(
                    sections.debug_addr,
                    relocations,
                    unit,
                    index,
    ...
    H: object::read::elf::FileHeader<Endian = object::LittleEndian>,
{
    ...
    let dwarf: gimli::Dwarf<ReaderWrapper<gimli::read::EndianRcSlice<gimli::LittleEndian>>> =
    ...
    let mut units = Vec::new();
    {
        ...
        while let Some(header) = iter.next()? {
            ...
            let low_pc = extract_symbolic_low_pc(
                &dwarf,
                &sections,
                relocations,
                section_map,
