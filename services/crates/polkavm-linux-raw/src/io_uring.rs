...
impl IoUring {
    pub fn new(queue_size: u32) -> Result<Self, linux_raw::Error> {
        ...
        let sring_size =
            params.sq_off.array + params.sq_entries * core::mem::size_of::<u32>() as u32;
        let cring_size = params.cq_off.cqes
            + params.cq_entries * core::mem::size_of::<linux_raw::io_uring_cqe>() as u32;
        ...
        let submit_head_pointer = unsafe {
            ring_map
                .as_ptr()
                .byte_add(params.sq_off.head as usize)
                .cast::<AtomicU32>()
        };
        let submit_tail_pointer = unsafe {
            ring_map
                .as_ptr()
                .byte_add(params.sq_off.tail as usize)
                .cast::<AtomicU32>()
        };
        let submit_capacity = unsafe {
            *ring_map
                .as_ptr()
                .byte_add(params.sq_off.ring_entries as usize)
                .cast::<u32>()
        };
        let array = unsafe {
            ring_map
                .as_mut_ptr()
                .byte_add(params.sq_off.array as usize)
                .cast::<u32>()
        };
        ...
        let completion_head_pointer = unsafe {
            ring_map
                .as_ptr()
                .byte_add(params.cq_off.head as usize)
                .cast::<AtomicU32>()
        };
        let completion_tail_pointer = unsafe {
            ring_map
                .as_ptr()
                .byte_add(params.cq_off.tail as usize)
                .cast::<AtomicU32>()
        };
        ...
        Ok(IoUring {
            ...
            submit_ring_mask: unsafe {
                *ring_map
                    .as_ptr()
                    .byte_add(params.sq_off.ring_mask as usize)
                    .cast::<u32>()
            },
            submit_capacity,
            ...
            completion_ring_mask: unsafe {
                *ring_map
                    .as_ptr()
                    .byte_add(params.cq_off.ring_mask as usize)
                    .cast::<u32>()
            },
    ...
    pub fn queue_read(
        ...
        if self.queue_length() >= self.queue_capacity() {
            return Err(linux_raw::Error::from(
                "no remaining capacity in the io_uring submission queue",
            ));
        }
    ...
    pub fn queue_futex_wait(
        ...
        unsafe {
            sqe.__bindgen_anon_6.__bindgen_anon_1.as_mut().addr3 =
                u64::from(linux_raw::FUTEX_BITSET_MATCH_ANY)
        };
        sqe.user_data = user_data;
    ...
    pub fn cancel_all_sync(&mut self) -> Result<(), linux_raw::Error> {
        let reg = linux_raw::io_uring_sync_cancel_reg {
            ...
            timeout: linux_raw::arch_bindings::__kernel_timespec {
                tv_sec: !0,
                tv_nsec: !0,
            },
            opcode: 0,
...
fn test_io_uring_read() {
    ...
    io_uring
        .queue_read(0x1234, fd.borrow(), buffer.as_mut_ptr(), 1)
        .unwrap();
    assert_eq!(io_uring.queue_length(), 1);
    io_uring
        .queue_read(0x1235, fd.borrow(), buffer[2..].as_mut_ptr(), 2)
        .unwrap();
...
fn get_kernel_version() -> Result<(u32, u32), linux_raw::Error> {
    ...
    let Ok(release) = unsafe { core::ffi::CStr::from_ptr(uname.release.as_ptr().cast()) }.to_str()
    else {
    ...
    let Some(major) = iter.next().and_then(|major| major.parse::<u32>().ok()) else {
        return Err(linux_raw::Error::from_str(
            "failed to extract the kernel's major version",
        ));
    };

    let Some(minor) = iter.next().and_then(|minor| minor.parse::<u32>().ok()) else {
        return Err(linux_raw::Error::from_str(
            "failed to extract the kernel's minor version",
        ));
    };
...
fn test_io_uring_futex_wake() {
    ...
    std::thread::spawn(move || {
        ...
        let mut io_uring = IoUring::new(2).unwrap();
        io_uring
            .queue_futex_wake_one(0x1234, &*futex_clone)
            .unwrap();
        unsafe {
            io_uring.submit_and_wait(1).unwrap();
        }
