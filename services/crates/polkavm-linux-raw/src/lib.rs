...
const UFFDIO_REGISTER: c_uint = iowr!(
    UFFDIO,
    crate::arch_bindings::_UFFDIO_REGISTER,
    uffdio_register
);
const UFFDIO_UNREGISTER: c_uint = ior!(
    UFFDIO,
    crate::arch_bindings::_UFFDIO_UNREGISTER,
    uffdio_range
);
...
const UFFDIO_ZEROPAGE: c_uint = iowr!(
    UFFDIO,
    crate::arch_bindings::_UFFDIO_ZEROPAGE,
    uffdio_zeropage
);
const UFFDIO_MOVE: c_uint = iowr!(UFFDIO, crate::arch_bindings::_UFFDIO_MOVE, uffdio_move);
const UFFDIO_WRITEPROTECT: c_uint = iowr!(
    UFFDIO,
    crate::arch_bindings::_UFFDIO_WRITEPROTECT,
    uffdio_writeprotect
);
const UFFDIO_CONTINUE: c_uint = iowr!(
    UFFDIO,
    crate::arch_bindings::_UFFDIO_CONTINUE,
    uffdio_continue
);
...
pub fn sys_pipe2(flags: c_uint) -> Result<(Fd, Fd), Error> {
    ...
    Ok((
        Fd::from_raw_unchecked(pipes[0]),
        Fd::from_raw_unchecked(pipes[1]),
    ))
}
...
pub fn sys_sendmsg(fd: FdRef, message: &msghdr, flags: u32) -> Result<usize, Error> {
    let result =
        unsafe { syscall_readonly!(SYS_sendmsg, fd.raw(), message as *const msghdr, flags) };
...
pub unsafe fn sys_execveat(
    ...
    let result =
        unsafe { syscall_readonly!(SYS_execveat, dirfd, path.as_ptr(), argv, envp, flags) };
...
pub fn sys_ptrace_interrupt(pid: pid_t) -> Result<(), Error> {
    let result = unsafe {
        syscall_readonly!(
            SYS_ptrace,
            crate::arch_bindings::PTRACE_INTERRUPT,
            pid,
            0,
            0
        )
...
pub fn sys_ptrace_attach(pid: pid_t) -> Result<(), Error> {
    let result =
        unsafe { syscall_readonly!(SYS_ptrace, crate::arch_bindings::PTRACE_ATTACH, pid, 0, 0) };
...
pub fn sys_ptrace_seize(pid: pid_t) -> Result<(), Error> {
    let result =
        unsafe { syscall_readonly!(SYS_ptrace, crate::arch_bindings::PTRACE_SEIZE, pid, 0, 0) };
...
pub fn sys_ptrace_continue(pid: pid_t, signal: Option<u32>) -> Result<(), Error> {
    let result = unsafe {
        syscall_readonly!(
            SYS_ptrace,
            crate::arch_bindings::PTRACE_CONT,
            pid,
            0,
            signal.unwrap_or(0)
        )
...
pub fn sys_ptrace_detach(pid: pid_t) -> Result<(), Error> {
    let result =
        unsafe { syscall_readonly!(SYS_ptrace, crate::arch_bindings::PTRACE_DETACH, pid, 0, 0) };
...
pub fn sys_ptrace_getregs(pid: pid_t) -> Result<user_regs_struct, Error> {
    ...
    let result = unsafe {
        syscall!(
            SYS_ptrace,
            crate::arch_bindings::PTRACE_GETREGS,
            pid,
            0,
            output.as_mut_ptr()
        )
...
pub fn sys_ptrace_setregs(pid: pid_t, regs: &user_regs_struct) -> Result<(), Error> {
    ...
    let result = unsafe {
        syscall_readonly!(
            SYS_ptrace,
            crate::arch_bindings::PTRACE_SETREGS,
            pid,
            0,
            regs
        )
...
pub fn sys_prctl_cap_ambient_clear_all() -> Result<(), Error> {
    ...
    let result =
        unsafe { syscall_readonly!(SYS_prctl, PR_CAP_AMBIENT, PR_CAP_AMBIENT_CLEAR_ALL, 0, 0, 0) };
...
pub fn sys_capset(
    header: &__user_cap_header_struct,
...
pub unsafe fn sys_rt_sigaction(
    signal: u32,
    new_action: &kernel_sigaction,
    old_action: Option<&mut kernel_sigaction>,
) -> Result<(), Error> {
    let result = unsafe {
        syscall_readonly!(
            SYS_rt_sigaction,
            ...
            old_action.map_or(core::ptr::null_mut(), |old_action| old_action
                as *mut kernel_sigaction),
...
pub unsafe fn sys_rt_sigprocmask(
    how: u32,
    new_sigset: &kernel_sigset_t,
    old_sigset: Option<&mut kernel_sigset_t>,
) -> Result<(), Error> {
    let result = unsafe {
        syscall_readonly!(
            SYS_rt_sigprocmask,
            ...
            old_sigset.map_or(core::ptr::null_mut(), |old_sigset| old_sigset
                as *mut kernel_sigset_t),
            core::mem::size_of::<kernel_sigset_t>()
        )
    };
...
pub fn sys_clock_gettime(clock_id: u32) -> Result<Duration, Error> {
    ...
    let result =
        unsafe { syscall_readonly!(SYS_clock_gettime, clock_id, core::ptr::addr_of_mut!(output)) };
            ...
            if self.position < self.bytes_available {
                let dirent = unsafe {
                    core::ptr::read_unaligned(
                        self.buffer.as_ptr().add(self.position).cast::<Dirent64>(),
                    )
                };
...
pub fn sendfd(socket: FdRef, fd: FdRef) -> Result<(), Error> {
    ...
    unsafe {
        ...
        core::ptr::write_unaligned(
            CMSG_DATA(buffer.as_mut_ptr().cast::<cmsghdr>()).cast::<c_int>(),
            fd.raw(),
        );
    }
...
pub fn recvfd(socket: FdRef) -> Result<Fd, Error> {
    ...
    if count != core::mem::size_of::<c_int>() {
        return Err(Error::from_str(
            "recvfd failed: received unexpected number of bytes",
        ));
    }

    if header.msg_controllen != CMSG_SPACE(core::mem::size_of::<c_int>()) {
        return Err(Error::from_str(
            "recvfd failed: invalid control message size",
        ));
    }
    ...
    if control_header.cmsg_level != SOL_SOCKET {
        return Err(Error::from_str(
            "recvfd failed: invalid control message level",
        ));
    }

    if control_header.cmsg_type != SCM_RIGHTS {
        return Err(Error::from_str(
            "recvfd failed: invalid control message type",
        ));
    }
