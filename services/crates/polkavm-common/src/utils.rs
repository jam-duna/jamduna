...
impl ArcBytes {
    ...
    pub(crate) fn subslice(&self, subrange: Range<usize>) -> Self {
        ...
        ArcBytes {
            ...
            pointer: unsafe {
                core::ptr::NonNull::new_unchecked(self.pointer.as_ptr().add(subrange.start))
            },
            length,
...
fn test_parse_immediate() {
    // "special cases"
    assert_eq!(
        parse_immediate("0xffffffff"),
        Some(ParsedImmediate::U64(0xffffffff))
    );
    assert_eq!(
        parse_immediate("0xffffffff87654321"),
        Some(ParsedImmediate::U32(0x87654321))
    );
    assert_eq!(
        parse_immediate("0x80000075"),
        Some(ParsedImmediate::U64(0x80000075))
    );
    // "normal cases"
    assert_eq!(
        parse_immediate("0x1234"),
        Some(ParsedImmediate::U32(0x1234))
    );
    assert_eq!(
        parse_immediate("0x12345678"),
        Some(ParsedImmediate::U32(0x12345678))
    );
    assert_eq!(
        parse_immediate("0x1234567890"),
        Some(ParsedImmediate::U64(0x1234567890))
    );
    assert_eq!(
        parse_immediate("-1"),
        Some(ParsedImmediate::U32(0xffffffff))
    );
    assert_eq!(
        parse_immediate("-2"),
        Some(ParsedImmediate::U32(0xfffffffe))
    );
    assert_eq!(
        parse_immediate("i64 0xffffffff"),
        Some(ParsedImmediate::U64(0xffffffff))
    );
    assert_eq!(
        parse_immediate("0xdeadbeef"),
        Some(ParsedImmediate::U64(0xdeadbeef))
    );
    assert_eq!(
    ...
    assert_eq!(
        parse_immediate("0xf000000e").map(Into::into),
        Some(0xf000000eu64)
    );
    assert_eq!(
