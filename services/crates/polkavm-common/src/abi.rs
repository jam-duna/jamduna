...
impl MemoryMapBuilder {
    ...
    pub fn build(&self) -> Result<MemoryMap, &'static str> {
        ...
        let Some(ro_data_address_space) =
            align_to_next_page_u64(u64::from(VM_MAX_PAGE_SIZE), u64::from(ro_data_size))
        else {
        ...
        let Some(rw_data_address_space) =
            align_to_next_page_u64(u64::from(VM_MAX_PAGE_SIZE), u64::from(rw_data_size))
        else {
        ...
        let Some(stack_address_space) =
            align_to_next_page_u64(u64::from(VM_MAX_PAGE_SIZE), u64::from(stack_size))
        else {
        ...
        let Some(aux_data_address_space) =
            align_to_next_page_u64(u64::from(VM_MAX_PAGE_SIZE), u64::from(aux_data_size))
        else {
...
fn test_memory_map() {
...

    {
        ...
        assert_eq!(
            map.rw_data_address(),
            map.ro_data_address() + VM_MAX_PAGE_SIZE + max_size
        );
        assert_eq!(map.rw_data_size(), 0);
        assert_eq!(
            map.stack_address_high(),
            VM_ADDRESS_SPACE_TOP - VM_MAX_PAGE_SIZE
        );
        assert_eq!(
            map.stack_address_low(),
            VM_ADDRESS_SPACE_TOP - VM_MAX_PAGE_SIZE
        );
...

    {
        ...
        assert_eq!(
            map.stack_address_low(),
            VM_ADDRESS_SPACE_TOP - VM_MAX_PAGE_SIZE - max_size
        );
...
mod kani {
    ...
    fn memory_map() {
        ...
        let total_size = align_to_next_page_u64(u64::from(VM_MAX_PAGE_SIZE), ro_data_size as u64)
            .unwrap()
