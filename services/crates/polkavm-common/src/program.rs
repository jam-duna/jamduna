use crate::abi::{
    VM_CODE_ADDRESS_ALIGNMENT, VM_MAXIMUM_CODE_SIZE, VM_MAXIMUM_IMPORT_COUNT,
...
impl RawReg {
    ...
    pub const fn get(self) -> Reg {
        ...
        let Some(reg) = Reg::from_raw(value) else {
            unreachable!()
        };
        reg
    }
    ...
    debug_assert!(
        opcode_visitor
            ...
            || !is_jump_target_valid(
                opcode_visitor.instruction_set(),
                code,
                bitmask,
                offset + skip + 1
            )
    ...
    T: OpcodeVisitor,
{
    ...
    let is_next_instruction_invalid =
        skip == 24 && !get_bit_for_offset(bitmask, code.len(), next_offset);
    (next_offset, result, is_next_instruction_invalid)
}

#[cfg_attr(not(debug_assertions), inline(always))]
#[cold]
fn visitor_step_invalid_instruction<T>(
    state: &mut <T as OpcodeVisitor>::State,
    offset: u32,
    opcode_visitor: T,
) -> T::ReturnTy
    ...
    const fn build(offset: i32) -> Self {
        ...
        while skip <= 0b11111 {
            ...
            while aux <= 0b111 {
                ...
                let imm2_length =
                    clamp_i32(0..=4, skip as i32 - imm1_length as i32 - offset) as u32;
...
#[inline(always)]
pub fn read_args_reg_imm_offset(
    chunk: u128,
...
mod kani {
    ...
    fn verify_read_args_imm() {
        ...
        assert_eq!(
            super::read_args_imm(chunk, skip),
            simple_read_args_imm(&code, skip)
        );
    }
    ...
    fn verify_read_args_imm2() {
        ...
        let (code, chunk, skip) = args!();
        assert_eq!(
            super::read_args_imm2(chunk, skip),
            simple_read_args_imm2(&code, skip as i32)
        );
    }
    ...
    fn verify_read_args_reg_imm2() {
        ...
        assert_eq!(
            (reg, imm1, imm2),
            simple_read_args_reg_imm2(&code, skip as i32)
        );
    }
    ...
    fn verify_read_args_regs2_imm2() {
        ...
        assert_eq!(
            (reg1, reg2, imm1, imm2),
            simple_read_args_regs2_imm2(&code, skip as i32)
        )
    }
...
impl Instruction {
    pub fn display<'a>(self, format: &'a InstructionFormat<'a>) -> impl core::fmt::Display + 'a {
        ...
        impl<'a, 'b> core::fmt::Display for Inner<'a, 'b> {
            fn fmt(&self, fmt: &mut core::fmt::Formatter) -> core::fmt::Result {
                self.instruction.visit(&mut InstructionFormatter {
                    format: self.format,
                    fmt,
                })
            }
...
pub struct InstructionFormat<'a> {
    ...
    pub jump_target_formatter:
        Option<&'a dyn Fn(u32, &mut core::fmt::Formatter) -> core::fmt::Result>,
    pub is_64_bit: bool,
}
    ...
    fn fmt(&self, fmt: &mut core::fmt::Formatter) -> core::fmt::Result {
        match self.0 {
            ...
            ProgramParseErrorKind::UnsupportedVersion { version } => {
                write!(
                    fmt,
                    "failed to parse program blob: unsupported version: {}",
                    version
                )
            }
    ...
    T: ?Sized + AsRef<[u8]>,
{
    ...
    fn read_varint(&mut self) -> Result<u32, ProgramParseError> {
        ...
        let Some((length, value)) = read_varint(&self.blob.as_ref()[self.position..], first_byte)
        else {
...
impl<'a> Reader<'a, ArcBytes> {
    ...
    fn read_section_as_bytes(
        &mut self,
        out_section: &mut u8,
        expected_section: u8,
    ) -> Result<ArcBytes, ProgramParseError> {
...
impl<'a> Imports<'a> {
    ...
    pub fn get(&self, index: u32) -> Option<ProgramSymbol<&'a [u8]>> {
        ...
        let xs = self
            .offsets
            .get(offset_start as usize..offset_end as usize)?;
        let offset = u32::from_le_bytes([xs[0], xs[1], xs[2], xs[3]]) as usize;
        let next_offset = offset_end
            .checked_add(4)
            .and_then(|next_offset_end| {
                self.offsets
                    .get(offset_end as usize..next_offset_end as usize)
            })
            .map_or(self.symbols.len(), |xs| {
                u32::from_le_bytes([xs[0], xs[1], xs[2], xs[3]]) as usize
            });
...
fn test_get_previous_instruction_skip() {
    ...
    assert_eq!(
        get_previous_instruction_skip(&[0b00000001, 0b00000001], 8),
        Some(7)
    );
    assert_eq!(
        get_previous_instruction_skip(&[0b00000001, 0b00000000], 8),
        Some(7)
    );
}
    ...
    I: InstructionSet,
{
    ...
    let Some(opcode) = instruction_set.opcode_from_u8(code[offset as usize - skip as usize - 1])
    else {
...
#[inline]
pub fn find_start_of_basic_block<I>(
    instruction_set: I,
    code: &[u8],
    bitmask: &[u8],
    mut offset: u32,
...
fn test_is_jump_target_valid() {
    fn assert_get_previous_instruction_skip_matches_instruction_parser(
        code: &[u8],
        bitmask: &[u8],
    ) {
        for instruction in
            Instructions::new(DefaultInstructionSet::default(), code, bitmask, 0, false)
        {
            match instruction.kind {
                Instruction::trap => {
                    ...
                    if let Some(skip) = skip {
                        ...
                        for skip in 0..=24 {
                            let Some(previous_offset) = instruction.offset.0.checked_sub(skip + 1)
                            else {
                                continue;
                            };
                            assert_eq!(
...
fn parse_bitmask_slow(bitmask: &[u8], code_length: usize, offset: u32) -> (u32, bool) {
    ...
    while let Some(&byte) = bitmask.get(offset >> 3) {
        ...
        if mask == 0 {
            ...
            is_next_instruction_invalid =
                offset >= code_length || (offset - origin) > BITMASK_MAX as usize;
        }
        break;
    }
...
pub(crate) fn parse_bitmask_fast(bitmask: &[u8], mut offset: u32) -> Option<u32> {
    ...
    let shift = offset & 7;
    let mask: u32 = (u32::from_le_bytes([bitmask[0], bitmask[1], bitmask[2], bitmask[3]]) >> shift)
        | (1 << BITMASK_MAX);
    ...
    I: InstructionSet,
{
    ...
    fn new(
        ...
        let (offset, invalid_offset) = if is_valid {
            ...
            let next_offset = find_next_offset_unbounded(bitmask, code.len() as u32, offset);
            debug_assert!(
                next_offset as usize == code.len()
                    || get_bit_for_offset(bitmask, code.len(), next_offset),
                "bit at {offset} is zero"
            );
    ...
    I: InstructionSet,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        debug_assert!(
            get_bit_for_offset(self.bitmask, self.code.len(), offset),
            "bit at {offset} is zero"
        );
        ...
        if !is_next_instruction_invalid {
            ...
            debug_assert!(
                self.offset as usize == self.code.len()
                    || get_bit_for_offset(self.bitmask, self.code.len(), self.offset),
                "bit at {} is zero",
                self.offset
            );
        } else {
            if next_offset as usize == self.code.len() {
                ...
                self.offset =
                    find_next_offset_unbounded(self.bitmask, self.code.len() as u32, next_offset);
                debug_assert!(
                    self.offset as usize == self.code.len()
                        || get_bit_for_offset(self.bitmask, self.code.len(), self.offset),
                    "bit at {} is zero",
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        (
            0,
            Some(self.code.len() - core::cmp::min(self.offset() as usize, self.code.len())),
        )
    }
}
...
impl ProgramParts {
    pub fn from_bytes(blob: ArcBytes) -> Result<Self, ProgramParseError> {
        ...
        let blob_len =
            BlobLen::from_le_bytes(reader.read_slice(BLOB_LEN_SIZE)?.try_into().unwrap());
        ...
        if section == SECTION_IMPORTS {
            ...
            let Some(import_offsets_size) = import_count.checked_mul(4) else {
                return Err(ProgramParseError(ProgramParseErrorKind::Other(
                    "the imports section is invalid",
                )));
            };

            parts.import_offsets = reader.read_slice_as_bytes(import_offsets_size as usize)?;
            let Some(import_symbols_size) =
                section_length.checked_sub(reader.position - section_start)
            else {
                return Err(ProgramParseError(ProgramParseErrorKind::Other(
                    "the imports section is invalid",
                )));
        ...
        parts.code_and_jump_table =
            reader.read_section_as_bytes(&mut section, SECTION_CODE_AND_JUMP_TABLE)?;
        parts.debug_strings =
            reader.read_section_as_bytes(&mut section, SECTION_OPT_DEBUG_STRINGS)?;
        parts.debug_line_programs =
            reader.read_section_as_bytes(&mut section, SECTION_OPT_DEBUG_LINE_PROGRAMS)?;
        parts.debug_line_program_ranges =
            reader.read_section_as_bytes(&mut section, SECTION_OPT_DEBUG_LINE_PROGRAM_RANGES)?;
...
impl ProgramBlob {
    ...
    pub fn blob_length(raw_blob: &[u8]) -> Option<BlobLen> {
        ...
        Some(BlobLen::from_le_bytes(
            raw_blob[BLOB_LEN_OFFSET..end].try_into().unwrap(),
        ))
    }
    ...
    pub fn from_parts(parts: ProgramParts) -> Result<Self, ProgramParseError> {
...

        {
            ...
            if code_length > VM_MAXIMUM_CODE_SIZE {
                return Err(ProgramParseError(ProgramParseErrorKind::Other(
                    "the code section is too long",
                )));
            }

            if !matches!(jump_table_entry_size, 0..=4) {
                return Err(ProgramParseError(ProgramParseErrorKind::Other(
                    "invalid jump table entry size",
                )));
            }

            let Some(jump_table_length) =
                jump_table_entry_count.checked_mul(u32::from(jump_table_entry_size))
            else {
                return Err(ProgramParseError(ProgramParseErrorKind::Other(
                    "the jump table is too long",
                )));
            };
            ...
            let bitmask_length =
                parts.code_and_jump_table.len() - (reader.position - initial_position);
        ...
        #[cfg(feature = "unique-id")]
        {
            static ID_COUNTER: core::sync::atomic::AtomicU64 =
...
impl<'a> FrameInfo<'a> {
    ...
    pub fn function_name_without_namespace(&self) -> Result<Option<&str>, ProgramParseError> {
        let function_name = self
            .blob
            .get_debug_string(self.inner.function_name_offset)?;
    ...
    pub fn location(&self) -> Result<Option<SourceLocation<'_>>, ProgramParseError> {
...
impl<'a> LineProgram<'a> {
    ...
    pub fn run(&mut self) -> Result<Option<RegionInfo<'_>>, ProgramParseError> {
        ...
        for _ in 0..INSTRUCTION_LIMIT_PER_REGION {
            ...
            let range =
                ProgramCounter(self.program_counter)..ProgramCounter(self.program_counter + count);
