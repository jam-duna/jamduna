                ...
                if let Some(index) = line.find(' ') {
                    ...
                    let (kind, lhs, rhs) = match (condition.lhs, condition.rhs) {
                        (RegImm::Reg(lhs), RegImm::Reg(rhs)) => {
                            emit_and_continue!(MaybeInstruction::Branch(
                                label.to_owned(),
                                condition.kind,
                                lhs,
                                rhs
                        ...
                        (RegImm::Imm(lhs), RegImm::Reg(rhs)) => {
                            (condition.kind.reverse_operands(), rhs, lhs)
                        }
                        (RegImm::Imm(_), RegImm::Imm(_)) => {
                            return Err(format!(
                                "cannot parse line {nth_line}: both arguments cannot be immediates"
                            ));
                        }
                    };

                    emit_and_continue!(MaybeInstruction::BranchImm(
                        label.to_owned(),
                        kind,
                        lhs,
                        rhs
        ...
        if let Some(index) = line.find('=') {
            ...
            if let Some(dst) = parse_reg(lhs) {
                if let Some(index) = rhs.find(',') {
                    if let Some(value) =
                        parse_immediate(&rhs[..index]).and_then(|value| value.try_into().ok())
                    {
                        if let Some(line) = rhs[index + 1..].trim().strip_prefix("jump") {
                            if let Some(label) = line.trim().strip_prefix('@') {
                                emit_and_continue!(MaybeInstruction::LoadImmAndJump(
                                    dst,
                                    value,
                                    label.to_owned()
                                ));
                            ...
                            if let Some((base, offset)) = parse_indirect_memory_access(line) {
                                ...
                                if dst == base {
                                    return Err(format!(
                                        "cannot parse line {nth_line}, expected: \"{instruction}\""
                                    ));
                                }
                                ...
                                (condition.lhs, condition.rhs)
                            {
                                let inst = match (src, condition.kind) {
                                    ...
                                    (RegImm::Imm(src), ConditionKind::Eq) => {
                                        Some(Instruction::cmov_if_zero_imm(
                                            dst.into(),
                                            cond.into(),
                                            src as u32,
                                        ))
                ...
                if let Some((index, op_len, op)) = operation {
                            ...
                            match op_marker {
                                OpMarker::I32 => {
                                    emit_and_continue!(match op {
                                        ...
                                        Op::DivUnsigned =>
                                            Instruction::div_unsigned_32(dst, src1, src2),
                                        Op::DivSigned =>
                                            Instruction::div_signed_32(dst, src1, src2),
                                        Op::RemUnsigned =>
                                            Instruction::rem_unsigned_32(dst, src1, src2),
                                        Op::RemSigned =>
                                            Instruction::rem_signed_32(dst, src1, src2),
                                        Op::LessUnsigned => {
                                            return Err(format!("cannot parse line {nth_line}: i32 not supported for operation"));
                                        }
                                        Op::LessSigned => {
                                        ...
                                        Op::ShiftLeft =>
                                            Instruction::shift_logical_left_32(dst, src1, src2),
                                        Op::ShiftRight =>
                                            Instruction::shift_logical_right_32(dst, src1, src2),
                                        Op::ShiftArithmeticRight =>
                                            Instruction::shift_arithmetic_right_32(dst, src1, src2),
                                    });
                                }
                                OpMarker::NONE => {
                                    emit_and_continue!(match op {
                                        ...
                                        Op::DivUnsigned =>
                                            Instruction::div_unsigned_64(dst, src1, src2),
                                        Op::DivSigned =>
                                            Instruction::div_signed_64(dst, src1, src2),
                                        Op::RemUnsigned =>
                                            Instruction::rem_unsigned_64(dst, src1, src2),
                                        Op::RemSigned =>
                                            Instruction::rem_signed_64(dst, src1, src2),
                                        Op::LessUnsigned =>
                                            Instruction::set_less_than_unsigned(dst, src1, src2),
                                        Op::LessSigned =>
                                            Instruction::set_less_than_signed(dst, src1, src2),
                                        Op::GreaterUnsigned =>
                                            Instruction::set_less_than_unsigned(dst, src2, src1),
                                        Op::GreaterSigned =>
                                            Instruction::set_less_than_signed(dst, src2, src1),
                                        Op::ShiftLeft =>
                                            Instruction::shift_logical_left_64(dst, src1, src2),
                                        Op::ShiftRight =>
                                            Instruction::shift_logical_right_64(dst, src1, src2),
                                        Op::ShiftArithmeticRight =>
                                            Instruction::shift_arithmetic_right_64(dst, src1, src2),
                                    });
                                }
                            }
                        } else if let Some(src2) =
                            parse_immediate(src2).and_then(|value| value.try_into().ok())
                        {
                            let dst = dst.into();
                            let src1 = src1.into();
                            match op_marker {
                                OpMarker::I32 => {
                                    emit_and_continue!(match op {
                                        ...
                                        Op::ShiftLeft =>
                                            Instruction::shift_logical_left_imm_32(dst, src1, src2),
                                        Op::ShiftRight =>
                                            Instruction::shift_logical_right_imm_32(dst, src1, src2),
                                        Op::ShiftArithmeticRight =>
                                            Instruction::shift_arithmetic_right_imm_32(
                                                dst, src1, src2
                                            ),
                                    });
                                }
                                OpMarker::NONE => {
                                    emit_and_continue!(match op {
                                        ...
                                        Op::LessUnsigned =>
                                            Instruction::set_less_than_unsigned_imm(dst, src1, src2),
                                        Op::LessSigned =>
                                            Instruction::set_less_than_signed_imm(dst, src1, src2),
                                        Op::GreaterUnsigned =>
                                            Instruction::set_greater_than_unsigned_imm(
                                                dst, src1, src2
                                            ),
                                        Op::GreaterSigned =>
                                            Instruction::set_greater_than_signed_imm(
                                                dst, src1, src2
                                            ),
                                        Op::ShiftLeft =>
                                        ...
                                        Op::ShiftArithmeticRight =>
                                            Instruction::shift_arithmetic_right_imm_64(
                                                dst, src1, src2
                                            ),
                                    });
                                }
                    ...
                    } else if let Some(src1) =
                        parse_immediate(src1).and_then(|value| value.try_into().ok())
                    {
                        if let Some(src2) = parse_reg(src2) {
                            let dst = dst.into();
                            let src2 = src2.into();
                            match op_marker {
                                OpMarker::I32 => {
                                    emit_and_continue!(match op {
                                        ...
                                        Op::Sub =>
                                            Instruction::negate_and_add_imm_32(dst, src2, src1),
                                        Op::And => {
                                            return Err(format!("cannot parse line {nth_line}: i32 not supported for operation"));
                                        }
                                        Op::Xor => {
                                        ...
                                        Op::ShiftLeft =>
                                            Instruction::shift_logical_left_imm_alt_32(
                                                dst, src2, src1
                                            ),
                                        Op::ShiftRight =>
                                            Instruction::shift_logical_right_imm_alt_32(
                                                dst, src2, src1
                                            ),
                                        Op::ShiftArithmeticRight =>
                                            Instruction::shift_arithmetic_right_imm_alt_32(
                                                dst, src2, src1
                                            ),
                                    });
                                }
                                OpMarker::NONE => {
                                    emit_and_continue!(match op {
                                        ...
                                        Op::Sub =>
                                            Instruction::negate_and_add_imm_64(dst, src2, src1),
                                        ...
                                        Op::LessSigned => Instruction::set_greater_than_signed_imm(
                                            dst, src2, src1
                                        ),
                                        Op::GreaterUnsigned =>
                                            Instruction::set_less_than_unsigned_imm(dst, src2, src1),
                                        Op::GreaterSigned =>
                                            Instruction::set_less_than_signed_imm(dst, src2, src1),
                                        Op::ShiftLeft =>
                                            Instruction::shift_logical_left_imm_alt_64(
                                                dst, src2, src1
                                            ),
                                        Op::ShiftRight =>
                                            Instruction::shift_logical_right_imm_alt_64(
                                                dst, src2, src1
                                            ),
                                        Op::ShiftArithmeticRight =>
                                            Instruction::shift_arithmetic_right_imm_alt_64(
                                                dst, src2, src1
                                            ),
                                    });
                                }
            ...
            if let Some((kind, lhs)) = store_kind {
                if let Some(offset) = parse_absolute_memory_access(lhs) {
                    ...
                    } else if let Some(rhs) =
                        parse_immediate(rhs).and_then(|value| value.try_into().ok())
                    {
                        emit_and_continue!(match kind {
                            StoreKind::U8 => match u8::try_from(rhs) {
                                ...
                                Err(_) =>
                                    return Err(format!(
                                        "cannot parse line {nth_line}: immediate larger than u8"
                                    )),
                            },
                            StoreKind::U16 => match u16::try_from(rhs) {
                                ...
                                Err(_) =>
                                    return Err(format!(
                                        "cannot parse line {nth_line}: immediate larger than u16"
                                    )),
                            },
                        ...
                        parse_immediate(rhs).and_then(|value| value.try_into().ok())
                    {
                        emit_and_continue!(match kind {
                            ...
                            StoreKind::U32 =>
                                Instruction::store_imm_indirect_u32(base, offset, rhs),
                            StoreKind::U64 =>
                                Instruction::store_imm_indirect_u64(base, offset, rhs),
                        });
                    }
                }
            }
    ...
    for instruction in instructions {
        match instruction {
            ...
            MaybeInstruction::LoadImmAndJump(dst, value, label) => {
                ...
                code.push(Instruction::load_imm_and_jump(
                    dst.into(),
                    value,
                    target_index,
                ));
            ...
            MaybeInstruction::Branch(label, kind, lhs, rhs) => {
                ...
                let instruction = match kind {
                    ...
                    ConditionKind::LessSigned => {
                        Instruction::branch_less_signed(lhs, rhs, target_index)
                    }
                    ConditionKind::LessUnsigned => {
                        Instruction::branch_less_unsigned(lhs, rhs, target_index)
                    }
                    ConditionKind::GreaterOrEqualSigned => {
                        Instruction::branch_greater_or_equal_signed(lhs, rhs, target_index)
                    }
                    ConditionKind::GreaterOrEqualUnsigned => {
                        Instruction::branch_greater_or_equal_unsigned(lhs, rhs, target_index)
                    }

                    ConditionKind::LessOrEqualSigned => {
                        Instruction::branch_greater_or_equal_signed(rhs, lhs, target_index)
                    }
                    ConditionKind::LessOrEqualUnsigned => {
                        Instruction::branch_greater_or_equal_unsigned(rhs, lhs, target_index)
                    }
                    ConditionKind::GreaterSigned => {
                        Instruction::branch_less_signed(rhs, lhs, target_index)
                    }
                    ConditionKind::GreaterUnsigned => {
                        Instruction::branch_less_unsigned(rhs, lhs, target_index)
                    }
                };
                code.push(instruction);
            }
            MaybeInstruction::BranchImm(label, kind, lhs, rhs) => {
                ...
                let instruction = match kind {
                    ...
                    ConditionKind::LessSigned => {
                        Instruction::branch_less_signed_imm(lhs, rhs, target_index)
                    }
                    ConditionKind::LessUnsigned => {
                        Instruction::branch_less_unsigned_imm(lhs, rhs, target_index)
                    }
                    ConditionKind::GreaterOrEqualSigned => {
                        Instruction::branch_greater_or_equal_signed_imm(lhs, rhs, target_index)
                    }
                    ConditionKind::GreaterOrEqualUnsigned => {
                        Instruction::branch_greater_or_equal_unsigned_imm(lhs, rhs, target_index)
                    }
                    ConditionKind::LessOrEqualSigned => {
                        Instruction::branch_less_or_equal_signed_imm(lhs, rhs, target_index)
                    }
                    ConditionKind::LessOrEqualUnsigned => {
                        Instruction::branch_less_or_equal_unsigned_imm(lhs, rhs, target_index)
                    }
                    ConditionKind::GreaterSigned => {
                        Instruction::branch_greater_signed_imm(lhs, rhs, target_index)
                    }
                    ConditionKind::GreaterUnsigned => {
                        Instruction::branch_greater_unsigned_imm(lhs, rhs, target_index)
                    }
                };
                code.push(instruction);
            }
...
fn assert_assembler(input: &str, expected_output: &str) {
    ...
    let expected_output_clean: Vec<_> = expected_output
        .trim()
        .split('\n')
        .map(|line| line.trim())
        .collect();
