use crate::program::{
    self, Instruction, ProgramCounter, ProgramSymbol, BLOB_LEN_OFFSET, BLOB_LEN_SIZE,
};
use alloc::boxed::Box;
use alloc::vec::Vec;
use core::ops::Range;
...
impl InstructionBuffer {
    ...
    fn new(position: u32, minimum_size: u8, instruction: Instruction) -> Self {
        ...
        if length < minimum_size {
            ...
            buffer.bytes[1..minimum_size].copy_from_slice(
                &u32::to_le_bytes(target.wrapping_sub(position))[..minimum_size - 1],
            );
            length = minimum_size;
        }
...
impl ProgramBlobBuilder {
    ...
    pub fn add_export_by_basic_block(&mut self, target_basic_block: u32, symbol: &[u8]) {
        self.exports
            .push((target_basic_block, ProgramSymbol::new(symbol.into())));
    }
    ...
    fn serialize_code(&self) -> SerializedCode {
        ...
        for (nth, symbol) in self.dispatch_table.iter().enumerate() {
            let Some(&(target_basic_block, _)) = self
                .exports
                .iter()
                .find(|(_, export_symbol)| symbol == export_symbol.as_bytes())
            else {
        ...
        for (nth_instruction, entry) in instructions.iter_mut().enumerate() {
            entry.target_nth_instruction = entry.instruction.target_mut().map(|target| {
                ...
                *target = position
                    .wrapping_add((target_nth_instruction as i32 - nth_instruction as i32) as u32);
            ...
            position = position
                .checked_add(entry.bytes.len() as u32)
        ...
        loop {
            ...
            for nth_instruction in 0..instructions.len() {
                let mut self_modified =
                    mutate(&mut instructions[nth_instruction].position, position);
                if let Some(target_nth_instruction) =
                    instructions[nth_instruction].target_nth_instruction
                {
                    let new_target = instructions[target_nth_instruction].position;
                    let old_target = instructions[nth_instruction]
                        .instruction
                        .target_mut()
                        .unwrap();
        ...
        for &target in &self.jump_table {
            ...
            jump_table_entry_size = core::cmp::max(
                jump_table_entry_size,
                offset.iter().take_while(|&&b| b != 0).count(),
            );
        }
        ...
        for target in jump_table_entries {
            output
                .jump_table
                .extend_from_slice(&target[..jump_table_entry_size]);
        }
        ...
        if cfg!(debug_assertions) {
            ...
            let parsed_instructions: Vec<_> = if self.is_64 {
                crate::program::Instructions::new_unbounded(
                    crate::program::ISA64_V1,
                    &output.code,
                    &output.bitmask,
                    0,
                )
            ...
            for (nth_instruction, (mut parsed, entry)) in
                parsed.into_iter().zip(instructions.into_iter()).enumerate()
            {
                let parsed_length = parsed.next_offset.0 - parsed.offset.0;
                if parsed.kind != entry.instruction
                    || entry.position != parsed.offset.0
                    || u32::from(entry.bytes.length) != parsed_length
                {
                    panic!(
                        concat!(
                        ...
                        parsed.offset.0,
                        parsed_length,
                        &output.code[parsed.offset.0 as usize
                            ..parsed.offset.0 as usize + parsed_length as usize],
                    );
                }
...
impl<'a> Writer<'a> {
    ...
    fn push_section_inplace(
        ...
        let payload_length: u32 = (self.buffer.len() - payload_position)
            .try_into()
            .expect("section size overflow");
        if payload_length == 0 {
        ...
        let length_length =
            crate::varint::write_varint(payload_length, &mut self.buffer[length_position..]);

        // Drain any excess length varint bytes.
        self.buffer.drain(
            length_position + length_length..length_position + crate::varint::MAX_VARINT_LENGTH,
        );
