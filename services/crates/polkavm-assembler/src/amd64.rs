...
impl MemOp {
    ...
    const fn needs_rex(self) -> bool {
        match self {
            ...
            MemOp::BaseIndexScaleOffset(_, _, base, index, _, _) => {
    ...
    fn fmt(&self, fmt: &mut core::fmt::Formatter) -> core::fmt::Result {
        let (segment, base, index, offset_reg_size, offset) = match self.simplify() {
            MemOp::BaseOffset(segment, reg_size, base, offset) => {
                (segment, Some((reg_size, base)), None, reg_size, offset)
            }
            MemOp::BaseIndexScaleOffset(segment, reg_size, base, index, scale, offset) => (
                segment,
                Some((reg_size, base)),
                Some((reg_size, index, scale)),
                reg_size,
                offset,
            ),
...
pub mod inst {
    ...
    fn alu_impl(op_reg2rm: u8, op_rm2reg: u8, opext: u8, operands: Operands) -> InstBuf {
        match operands {
            Operands::RegMem_Reg(size, dst, src) => {
                new_rm(op_reg2rm, size, dst, Some(src)).encode()
            }
            Operands::Reg_RegMem(size, dst, src) => {
                new_rm(op_rm2reg, size, src, Some(dst)).encode()
            }
            Operands::RegMem_Imm(dst, imm) => match imm {
                ImmKind::I8(imm) => Inst::new(0x80)
                    .rex_if(!matches!(
                        dst,
                        RegMem::Reg(Reg::rax | Reg::rcx | Reg::rdx | Reg::rbx)
                    ))
                    .imm8(imm),
    ...
    fn display_with_operands(
        ...
        match operands {
            Operands::RegMem_Reg(reg_size, dst, src) => match dst {
                RegMem::Mem(mem) => fmt.write_fmt(core::format_args!(
                    "{}, {}",
                    mem,
            ...
            Operands::RegMem_Imm(dst, imm) => {
                ...
                fmt.write_fmt(core::format_args!(
                    "{}, {imm}",
                    dst.display_without_prefix(imm.size())
                ))
            }
        }
    }
    ...
    impl GenerateTestValues for super::MemOp {
        fn generate_test_values(mut cb: impl FnMut(Self)) {
                ...
                super::RegSize::generate_test_values(|reg_size| {
                    super::Reg::generate_test_values(|base| {
                        i32::generate_test_values(|offset| {
                            cb(super::MemOp::BaseOffset(seg_reg, reg_size, base, offset))
                        })
                    })
                })
            });
                            ...
                            super::Scale::generate_test_values(|scale| {
                                i32::generate_test_values(|offset| {
                                    cb(super::MemOp::BaseIndexScaleOffset(
                                        seg_reg, reg_size, base, index, scale, offset,
                                    ))
                                })
                            })
                        })
                        ...
                        super::Scale::generate_test_values(|scale| {
                            i32::generate_test_values(|offset| {
                                cb(super::MemOp::IndexScaleOffset(
                                    seg_reg, reg_size, base, scale, offset,
                                ))
                            })
                        })
                    })
            ...
            Option::<super::SegReg>::generate_test_values(|seg_reg| {
                super::RegSize::generate_test_values(|reg_size| {
                    i32::generate_test_values(|offset| {
                        cb(super::MemOp::Offset(seg_reg, reg_size, offset))
                    })
                })
            });
