...
impl TryFrom<&'_ ProgramBlob> for NativeCode {
    ...
    fn try_from(blob: &'_ ProgramBlob) -> Result<Self, Self::Error> {
        if !cfg!(target_arch = "x86_64") {
            return Err(
                "the selected disassembly format is not supported on this architecture".into(),
            );
        }
...
impl<'a> Disassembler<'a> {
    pub fn new(blob: &'a ProgramBlob, format: DisassemblyFormat) -> Result<Self, polkavm::Error> {
        let native = if matches!(
            format,
            DisassemblyFormat::Native | DisassemblyFormat::GuestAndNative
        ) {
    ...
    pub fn disassemble_into(&self, mut writer: impl Write) -> Result<(), polkavm::Error> {
        ...
        let mut instruction_offset_to_basic_block = HashMap::new();
        {
            ...
            for instruction in self.instructions() {
                if basic_block_started {
                    instruction_offset_to_basic_block
                        .insert(instruction.offset, basic_block_counter);
        ...
        for target_code_offset in self.blob.jump_table() {
            ...
            assert!(jump_table_map
                .insert(target_code_offset, jump_table_index)
                .is_none());
        }
        ...
        if self.emit_header {
            w!(
                "// RO data = {}/{} bytes",
                self.blob.ro_data().len(),
                self.blob.ro_data_size()
            );
            w!(
                "// RW data = {}/{} bytes",
                self.blob.rw_data().len(),
                self.blob.rw_data_size()
            );
        ...
        let format_jump_target = |target_offset: ProgramCounter, basic_block_counter: u32| {
            ...
            if let Some(gas_cost) = self
                .gas_cost_map
                .as_ref()
                .and_then(|map| map.get(&target_offset))
            {
        ...
        let jump_target_formatter = |target: u32, fmt: &mut core::fmt::Formatter| {
            if prefer_offset_jump_targets {
                write!(fmt, "{}", target)
            } else if let Some(basic_block_index) =
                instruction_offset_to_basic_block.get(&polkavm::ProgramCounter(target))
            {
        ...
        for (nth_instruction, instruction) in instructions.iter().copied().enumerate() {
            ...
            let length =
                core::cmp::min(instruction.next_offset.0, self.blob.code().len() as u32) - offset.0;
            let instruction = instruction.kind;
            let raw_bytes =
                &self.blob.code()[offset.0 as usize..offset.0 as usize + length as usize];

            let instruction_s = instruction.display(&disassembly_format);
            let instruction_s =
                if let polkavm_common::program::Instruction::ecalli(nth_import) = instruction {
                    ...
                    loop {
                        ...
                        if region.instruction_range().contains(&offset) {
                            ...
                            let full_name = match frame.full_name() {
                                ...
                                Err(error) => {
                                    return Err(
                                        format!("failed to parse line program: {error}").into()
                                    );
                                }
                            }
                            .to_string();
                ...
                if !matches!(self.format, DisassemblyFormat::DiffFriendly) {
                    ...
                    if self.show_raw_bytes {
                        w!(
                            "{:24} {}",
                            "",
                            format_jump_target(offset, basic_block_counter)
                        )
                    } else {
            ...
            if matches!(self.format, DisassemblyFormat::DiffFriendly) {
                ...
                self.format,
                DisassemblyFormat::Guest | DisassemblyFormat::GuestAndNative
            ) {
                if self.show_offsets {
            ...
            if matches!(
                ...
                let machine_next_code_position =
                    native.instruction_map[nth_instruction + 1].1 as usize;
                let length = machine_next_code_position - machine_code_position;
                if length != 0 {
                    let machine_code_chunk =
                        &native.machine_code[machine_code_position..machine_next_code_position];
