    ...
    pub fn spawn_and_load_module(
            ...
            .and_then(|cache| S::downcast_worker_cache(cache).reuse_sandbox())
        {
            ...
            S::spawn(global, &sandbox_config)
                ...
                .map_err(|error| {
                    error.context("instantiation failed: failed to create a sandbox")
                })?
        };
        ...
        sandbox
            ...
            .map_err(Error::from_display)
            .map_err(|error| {
                error.context("instantiation failed: failed to upload the program into the sandbox")
            })?;
    ...
    fn drop(&mut self) {
        if let Some(cache) = self.engine_state.sandbox_cache.as_ref() {
            ...
            cache.recycle_sandbox(|| {
                ...
                let global =
                    S::downcast_global_state(self.engine_state.sandbox_global.as_ref().unwrap());
    ...
    pub(crate) fn new(kind: SandboxKind, config: &Config) -> Result<Self, Error> {
        match kind {
            SandboxKind::Linux => {
                ...
                {
                    Ok(Self::Linux(
                        crate::sandbox::linux::GlobalState::new(config).map_err(|error| {
                            format!("failed to initialize Linux sandbox: {error}")
                        })?,
                    ))
                }
            ...
            SandboxKind::Generic => {
                ...
                {
                    Ok(Self::Generic(
                        crate::sandbox::generic::GlobalState::new(config).map_err(|error| {
                            format!("failed to initialize generic sandbox: {error}")
                        })?,
                    ))
                }
...
impl WorkerCacheKind {
    ...
    pub(crate) fn spawn(&self, global: &GlobalStateKind) -> Result<(), Error> {
        match self {
            #[cfg(target_os = "linux")]
            WorkerCacheKind::Linux(ref cache) => cache.spawn(
                crate::sandbox::linux::Sandbox::downcast_global_state(global),
            ),
            #[cfg(feature = "generic-sandbox")]
            WorkerCacheKind::Generic(ref cache) => cache.spawn(
                crate::sandbox::generic::Sandbox::downcast_global_state(global),
            ),
        }
    }
    ...
    S: Sandbox,
{
    ...
    fn spawn(&self, global: &S::GlobalState) -> Result<(), Error> {
        ...
        self.available_workers
            .store(sandboxes.len(), Ordering::Relaxed);
    ...
    fn recycle_sandbox(&self, get_sandbox: impl FnOnce() -> Option<S>) {
        ...
        loop {
            if let Err(new_count) = self.available_workers.compare_exchange(
                count,
                count + 1,
                Ordering::Relaxed,
                Ordering::Relaxed,
...
fn is_sandbox_logging_enabled() -> bool {
    cfg!(test)
        || log::log_enabled!(target: "polkavm", log::Level::Trace)
        || log::log_enabled!(target: "polkavm::zygote", log::Level::Trace)
}
