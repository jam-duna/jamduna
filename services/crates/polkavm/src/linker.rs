...
type FallbackHandlerArc<UserData, UserError> =
    Arc<dyn Fn(Caller<UserData>, u32) -> Result<(), UserError> + Send + Sync + 'static>;
...
impl<UserData, UserError> Linker<UserData, UserError> {
    ...
    pub fn define_fallback(
        &mut self,
        func: impl Fn(Caller<UserData>, u32) -> Result<(), UserError> + Send + Sync + 'static,
    ) {
    ...
    pub fn instantiate_pre(
        ...
        let mut imports: Vec<Option<CallFnArc<UserData, UserError>>> =
...
pub struct InstancePre<UserData = (), UserError = core::convert::Infallible>(
    Arc<InstancePreState<UserData, UserError>>,
);
