...
use polkavm_common::program::{
    FrameKind, ISA32_V1_NoSbrk, Imports, InstructionSet, Instructions, JumpTable, Opcode,
    ...
    pub fn new(config: &Config) -> Result<Self, Error> {
        ...
        if !config.sandboxing_enabled {
            if !config.allow_experimental {
                ...
                log::warn!(
                    "SECURITY SANDBOXING IS DISABLED; THIS IS UNSUPPORTED; YOU HAVE BEEN WARNED"
                );
            }
        }
...
impl Module {
    ...
    ) -> Instructions<'_, RuntimeInstructionSet> {
        self.state()
            .blob
    ...
    pub(crate) fn jump_table(&self) -> JumpTable<'_> {
    ...
    pub fn from_blob(
        ...
        macro_rules! compile_module {
            ($sandbox_kind:ident, $bitness_kind:ident, $isa:ident, $isa_no_sbrk:ident, $visitor_name:ident, $module_kind:ident) => {{
                type VisitorTy<'a> =
                    crate::compiler::CompilerVisitor<'a, $sandbox_kind, $bitness_kind>;
                let (mut visitor, aux) =
                    crate::compiler::CompilerVisitor::<$sandbox_kind, $bitness_kind>::new(
    ...
    pub fn calculate_gas_cost_for(&self, code_offset: ProgramCounter) -> Option<Gas> {
        ...
        let gas = crate::gas::calculate_for_block(
            self.state().cost_model.clone(),
            self.instructions_bounded_at(code_offset),
        );
        Some(i64::from(gas.0))
    }
