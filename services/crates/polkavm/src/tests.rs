...
use polkavm_common::program::{
    BlobLen, Reg::*, INTERPRETER_CACHE_ENTRY_SIZE, INTERPRETER_CACHE_RESERVED_ENTRIES,
};
...
fn bounded_interpreter_cache(config: Config) {
    ...
    for max_cache_size_bytes in 0..minimum_cache_size {
        log::debug!(
            "Testing with max_cache_size_bytes: {}",
...
fn pinky_impl(config: Config, is_64_bit: bool) {
    if (config.backend() == Some(crate::BackendKind::Interpreter) && cfg!(debug_assertions))
        || config.crosscheck()
    {
        return; // Too slow.
    }
...
impl TestInstance {
    ...
    pub fn call<FnArgs, FnResult>(
        ...
        name: &str,
        args: FnArgs,
    ) -> Result<FnResult, crate::CallError>
    where
...
fn test_blob_atomic_fetch_swap(args: TestBlobArgs) {
    ...
    assert_eq!(
        i.call::<(), u32>("atomic_fetch_swap_with_zero", ())
            .unwrap(),
        1000
    );
    assert_eq!(
...
fn test_blob_call_sbrk_from_guest(args: TestBlobArgs) {
    test_blob_call_sbrk_impl(args, |i, size| {
...
fn test_asm_reloc_hi_lo(config: Config, optimize: bool) {
    const BLOB_64: &[u8] =
        include_bytes!("../../../guest-programs/asm-tests/output/reloc_hi_lo_64.elf");
