...
fn run_program(idx: u8) -> u64 {
    match idx {
        ...
        3 => {
            ...
            if is_quadratic_residue(a, p) { 1 } else { 0 }
        }
        4 => {
            let p = ((get_random_number() % 50) | 1) + 2;
            if is_wilson_prime(p) { 1 } else { 0 }
        }
        5 => {
            let n = (get_random_number() % 1000) + 3 | 1;
            if solovay_strassen(n, 5) { 1 } else { 0 }
        }
        6 => {
            let n = ((get_random_number() % 500) + 4) * 2 + 1;
            fermat_factor(n).map(|(a, _)| a).unwrap_or(0)
        }
        7 => {
            diffie_hellman()
        }
        ...
        10 => {
            ...
            bin_packing_ffd(&mut items, 100) as u64
        }
        11 => {
        ...
        16 => {
            ...
            condition_number_2x2(a)
        }
        17 => {
            let n = (get_random_number() % 1000) | 1;
            pollard_rho_brent(n).unwrap_or(0)
        }
        18 => {
            ...
            tree.query(l, r) as u64
        }
        19 => {
            let (n, _e, _d) = rsa_keygen();
            n
        }
        20 => {
        ...
        22 => {
            ...
            legendre_symbol(a, p as i64) as u64
        }
        23 => {
            let p = (get_random_number() % 50) + 2;
            if lucas_lehmer(p) { 1 } else { 0 }
        }
        24 => {
            let n = get_random_number() % 20;
            let P = 1;
            ...
            let (U, _V) = lucas_sequence(n, P, Q, m);
            U as u64
        }
        25 => {
            let n = ((get_random_number() % 10_000) | 1) + 2;
            if baillie_psw(n) { 1 } else { 0 }
        }
        26 => {
        ...
        28 => {
            ...
            let (d1, _d2) = smith_normal_form_2x2(mat);
            d1 as u64
        }
        29 => {
            let mat = [
        ...
        30 => {
            ...
            let (r1, _r2) = lll_reduce_2d(b1, b2);
            r1.0 as u64
        }
        31 => {
        ...
        34 => {
            ...
            integer_log_mul(n, b) as u64
        }
        35 => {
            let n = (get_random_number() % 1_000_000) + 1;
            let b = (get_random_number() % 9) + 2;
            integer_log_div(n, b) as u64
        }
        36 => {
            let n = get_random_number() % 1_000_000;
            if is_perfect_square(n) { 1 } else { 0 }
        }
        37 => {
        ...
        51 => {
            let n = ((get_random_number() % 999) | 1) + 2;
            let a = (get_random_number() % (n as u64)) as i64;
            jacobi(a.try_into().unwrap(), (n as i64).try_into().unwrap()) as u64
        }
        52 => {
            let p = ((get_random_number() % 1000) | 1) + 2;
            let n = get_random_number() % p;
            cipolla(n, p).unwrap_or(0)
        }
        53..=u8::MAX => {
            0
        }
...
extern "C" fn refine(start_address: u64, length: u64) -> (u64, u64) {
    ...
    let payload = unsafe { slice::from_raw_parts(start_address as *const u8, length as usize) };
    ...
    for i in 0..pairs {
        ...
        let mut result_sum = 0 as u64;
        for iter in 0..iterations {
            result_sum = result_sum.wrapping_add(run_program(p_id));
            if iter % 1 ==0{
                call_log(3, None, &format!("algo refine PROGRAM_ID {} ITERATION {} partial_sum {}", program_id, iter, result_sum));
            }
        }
        call_log(
            2,
            None,
            &format!("algo run_refine {} ITERATIONS {} result_sum {}", program_id, iterations, result_sum),
        )
    }
