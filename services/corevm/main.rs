...
use utils::functions::{write_result, call_info};
...
extern "C" fn accumulate(start_address: u64, length: u64) -> (u64, u64) {
    ...
    if n == 1 {
        ...
        call_info("write deleted JAM, expect OK: 3");
        write_result(write_result1, 2);
        ...
        call_info("lookup hash(jam), expect OK: 3");
        ...
        call_info("bless, expect WHO");
        write_result(bless_who_result, 6);
    } else if n == 6 {
        let solicit_result = unsafe { solicit(jam_hash_address, jam_length) };
        call_info("solicit hash(jam), expect OK: insert t");
        write_result(solicit_result, 1);

        let query_jamhash_result = unsafe { query(jam_hash_address, jam_length) };
        call_info("query hash(jam), expect OK: 3+2^32*x");
        write_result(query_jamhash_result, 2);
        ...
        call_info("assign, expect OK");
        write_result(assign_ok_result, 5);
    } else if n == 7 {
        let forget_result = unsafe { forget(jam_hash_address, jam_length) };
        call_info("forget hash(jam), expect OK");
        write_result(forget_result, 1);
        ...
        call_info("read from DOT, expect NONE");
    ...
    call_info("info, expect OK");
    write_result(info_result, 8);

    let gas_result = unsafe { gas() };
    call_info("gas, expect OK: gas");
    ...
    if n % 3 == 0 {
        ...
        call_info("corevm panic");
    } else {
    }
    unsafe {
        oyield(output_address);
    }
    call_info("yield, expect OK"); 
