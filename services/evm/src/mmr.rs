...
impl MMR {
    ...
    fn compute_super_peak(hashes: &[Option<[u8; 32]>]) -> [u8; 32] {
        ...
        let non_nil_hashes: Vec<[u8; 32]> = hashes.iter().filter_map(|h| *h).collect();
    ...
    pub fn read_mmr(service_id: u32) -> Option<MMR> {
        use utils::{
            constants::{MMR_KEY, NONE, WHAT},
            functions::{log_debug, log_error},
        ...
        if result == WHAT {
            log_error(&format!(
                "❌ read_mmr: WHAT error (wrong mode), service_id={}",
                service_id
            ));
            return None;
        }
    ...
    pub fn write_mmr(&self) -> Option<[u8; 32]> {
        ...
        if result == WHAT || result == FULL {
            log_error(&format!(
                "❌ Failed to write MMR to storage, error code: {}",
                result
            ));
            return None;
        }
...
mod tests {
    ...
    fn test_super_peak_empty() {
        ...
        let expected: [u8; 32] = keccak256(&[]).into();
        assert_eq!(super_peak, expected);
    }
