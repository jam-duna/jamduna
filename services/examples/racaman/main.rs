...
extern "C" fn refine() -> u32 {
    ...
    if result == 0 {
        let past_n = u32::from_le_bytes(buffer[0..4].try_into().unwrap());
        let rac_past_n = u32::from_le_bytes(buffer[4..8].try_into().unwrap());
        let mut n = past_n + 1;

        let mut buffer2d = [[0u8; 12]; 100];
        ...
        for i in 1..=n {
            let result = unsafe { import(i, buffer2.as_mut_ptr(), buffer2.len() as u32) };
            if result == 0 {
                buffer2d[i as usize - 1].copy_from_slice(&buffer2);
            }
        }

        let an: u32;
        if rac_past_n >= n && !already_in_sequence(rac_past_n - n, &buffer2d, n) {
            an = rac_past_n - n;
        } else {
            an = rac_past_n + n;
        }
...
fn already_in_sequence(value: u32, buffer2d: &[[u8; 12]; 100], n: u32) -> bool {
    for i in 0..n as usize {
        let previous_value = u32::from_le_bytes(buffer2d[i][4..8].try_into().unwrap());
