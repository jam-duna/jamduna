...
extern "C" fn refine() -> u32 {
    ...
    let result = unsafe { import(1, buffer.as_mut_ptr(), buffer.len() as u32) };
    ...
    if result == 0 {
        ...
        let new_t_n = t_n.wrapping_add(t_n_minus_1).wrapping_add(t_n_minus_2);
        ...
        buffer[0..4].copy_from_slice(&2u32.to_le_bytes());
