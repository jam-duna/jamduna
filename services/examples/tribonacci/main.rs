...
extern "C" fn refine() -> u32 {
    ...
    if result == 0 {
        ...
        let trib_n = u32::from_le_bytes(buffer[4..8].try_into().unwrap());
        let trib_n_minus_1 = u32::from_le_bytes(buffer[8..12].try_into().unwrap());
        let trib_n_minus_2 = u32::from_le_bytes(buffer[12..16].try_into().unwrap());

        let new_trib_n = trib_n + trib_n_minus_1 + trib_n_minus_2;

        buffer[0..4].copy_from_slice(&(n + 1).to_le_bytes());
        buffer[4..8].copy_from_slice(&new_trib_n.to_le_bytes());
        buffer[8..12].copy_from_slice(&trib_n.to_le_bytes());
        buffer[12..16].copy_from_slice(&trib_n_minus_1.to_le_bytes());

    } else {
        buffer.copy_from_slice(&[
            2u8, 0u8, 0u8, 0u8,      // n = 2
            1u8, 0u8, 0u8, 0u8,      // trib_n = 1
            1u8, 0u8, 0u8, 0u8,      // trib_n_minus_1 = 1
            0u8, 0u8, 0u8, 0u8,      // trib_n_minus_2 = 0
        ]);
    }
