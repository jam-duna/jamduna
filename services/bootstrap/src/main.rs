...
extern "C" fn accumulate(start_address: u64, length: u64) -> (u64, u64) {
    // parse args
    let (_timeslot, _service_index, num_accumulate_inputs) = if let Some(args) = parse_accumulate_args(start_address, length) {
    ...
    let ptr = core::ptr::addr_of!(output_bytes_36) as *const u8 as u64;

    for i in 0..num_accumulate_inputs {
        ...
        let len: u32 = unsafe {
            ...
            let bytes = &(&(*arr))[32..36];
            u32::from_le_bytes(bytes.try_into().expect("slice length is exactly 4"))
        };

        unsafe {
            let arr = core::ptr::addr_of!(output_bytes_36);
            call_log(2, None, &format!("createService output_bytes_address {} {:x?} len={}", ptr, *arr, len));
        } 
        ...
        unsafe {
            let dest = core::ptr::addr_of_mut!(output_bytes_36);
            core::ptr::copy_nonoverlapping(result_bytes.as_ptr(), (*dest).as_mut_ptr().add(start), 4);
        }
    }
    (ptr, 32)
}
