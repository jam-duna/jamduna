...
impl CacheOnlyBackend {
    ...
    pub fn from_witness(service_id: u32, witness: &WitnessBundle) -> Result<Self> {
        ...
        for read in &witness.reads {
            if is_sparse_nullifier_key(&read.key) {
                read_cache.insert(read.key.clone(), read.value.clone());
                continue;
            }
            if !read.merkle_proof.is_empty() {
...
/// Record IssueBundle note commitments as state writes for builder-mode witness generation.
pub fn record_issue_bundle_commitments<B: StateBackend>(
    backend: &mut B,
    start_index: u64,
    commitments: &[[u8; 32]],
) -> Result<()> {
    for (offset, commitment) in commitments.iter().enumerate() {
        let key = format!("commitment_{}", start_index + offset as u64);
        backend.write_state(&key, commitment.to_vec())?;
    }
    Ok(())
}
        ...
        match self.execution_mode {
            ExecutionMode::Builder => {
                ...
                self.access_reads.push(StateRead {
                    key: key.to_string(),
                    value: value.clone(),
                    merkle_proof: Vec::new(), // No JAM proofs
                });
...
pub fn verify_merkle_proof(
    ...
    // Use JAM's production witness verification in all builds.
    use utils::effects::verify_merkle_proof as jam_verify_merkle_proof;
...
pub fn validate_witness_bundle(service_id: u32, witness: &WitnessBundle) -> Result<()> {
    ...
    // No write witnesses to validate (state roots validated via ZK proofs + frontier)
...
fn validate_witness_structure(witness: &WitnessBundle) -> Result<()> {
    ...
    // No write witnesses to validate
...
fn is_sparse_nullifier_key(key: &str) -> bool {
    let Some(hex_str) = key.strip_prefix("nullifier_") else {
        return false;
    };
    if hex_str.len() != 64 {
        return false;
    }
    hex_str.as_bytes().iter().all(|b| matches!(b, b'0'..=b'9' | b'a'..=b'f' | b'A'..=b'F'))
}
...
fn detect_duplicate_witnesses(witness: &WitnessBundle) -> Result<()> {
    ...
    // No write witnesses to check for duplicates

    Ok(())
}

/// Detect conflicting state transitions
fn detect_conflicting_transitions(_witness: &WitnessBundle) -> Result<()> {
    // No write witnesses, so no read/write conflicts possible
    Ok(())
}

/// Detect impossible state progressions
fn detect_impossible_progressions(_witness: &WitnessBundle) -> Result<()> {
    // State progression validation is handled by ZK proofs + frontier computations
    // Nullifier absence proofs are validated separately
...
mod tests {
    ...
    #[test]
    fn test_record_issue_bundle_commitments_builder_mode() {
        let mut backend = CacheOnlyBackend::builder_mode();
        let commitments = vec![[1u8; 32], [2u8; 32]];

        record_issue_bundle_commitments(&mut backend, 10, &commitments).unwrap();

        let (_reads, writes) = backend.get_access_log();
        assert_eq!(writes.len(), 2);
        assert_eq!(writes[0].key, "commitment_10");
        assert_eq!(writes[0].new_value, commitments[0].to_vec());
        assert_eq!(writes[1].key, "commitment_11");
        assert_eq!(writes[1].new_value, commitments[1].to_vec());
    }
