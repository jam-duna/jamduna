...
impl<G: UnknownOrderGroup, T: Eq + Hash> Accumulator<G, T> {
  ...
  /// Batch witness generation using GPU acceleration when available.
  ///
  /// Computes witnesses for multiple elements in parallel. For each element at index i,
  /// the witness is acc^(product_of_all_primes_except_i).
  ///
  /// # Arguments
  ///
  /// * `elems` - The elements to generate witnesses for.
  ///
  /// Returns a vector of witnesses, one for each element.
  pub fn batch_prove(&self, elems: &[T]) -> Vec<Witness<G, T>> {
    if elems.is_empty() {
      return Vec::new();
    }

    // Hash all elements to primes
    let primes: Vec<Integer> = elems.iter().map(hash_to_prime).collect();
    let n = primes.len();

    // For each witness_i, compute product of all primes except prime_i
    let exponents: Vec<Integer> = (0..n)
      .map(|i| {
        primes
          .iter()
          .enumerate()
    ...
    // Use batch GPU operation if available (RsaGpu overrides the default)
    let bases = vec![self.value.clone(); n];
    let witness_values = G::batch_exp(&bases, &exponents);
