...
mod rsa_gpu;
pub use rsa_gpu::{RsaGpu, RsaGpuElem};
...
pub trait Group: Clone + Debug + Eq + Hash + TypeRep + Send + Sync {
  ...
  /// Batch exponentiation operation (optional, with default CPU implementation).
  ///
  /// Computes `bases[i] ^ exps[i]` for all i. Groups may override this with
