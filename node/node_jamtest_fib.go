...
func fib3(nodeClient JNode, testServices map[string]*types.TestService, targetN int) {
	...
	for fibN := -1; fibN <= targetN; fibN++ {
		...
		if fibN == 3 || fibN == 6 {
			ctx, cancel := context.WithTimeout(context.Background(), RefineTimeout)
			defer cancel()
			err := nodeClient.SubmitAndWaitForPreimage(ctx, service0.ServiceCode, jam_key)
			if err != nil {
				log.Error(debugP, "SubmitAndWaitForPreimage", "err", err)
			} else {
				log.Info(module, "SubmitAndWaitForPreimage", "fibN", fibN, "id", "jam")
			}
		}
		...
		if fibN == -1 {
			...
			err := nodeClient.SubmitAndWaitForPreimage(ctx, service0.ServiceCode, fib2_child_code["corevm_child"].Code)
			if err != nil {
				log.Error(debugP, "SubmitAndWaitForPreimage", "err", err)
			} else {
				log.Info(module, "SubmitAndWaitForPreimage", "id", "corevm_child")
			}
		}
	}
}
