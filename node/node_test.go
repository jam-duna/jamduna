...
func TestWorkGuaranteeFIB(t *testing.T) {
		...
		for !new_service_found {
			...
			if stateDB != nil && stateDB.Block != nil {
				...
				if decoded_new_service_idx != 0 && (decoded_new_service_idx != previous_service_idx) {
					...
					fmt.Printf("Servcice %s get new service index %v\n", serviceName, service.ServiceCode)
				}
	...
	for _, service := range testServices {
		for validatorIdx, _ := range nodes {
			if validatorIdx != builderIdx {
				// fmt.Printf("SendPreimageAnnouncement %v service.CodeHash %v, len(service.Code)=%v\n", service.ServiceCode, service.CodeHash, len(service.Code))
				err = builderNode.peersInfo[uint16(validatorIdx)].SendPreimageAnnouncement(service.ServiceCode, service.CodeHash, uint32(len(service.Code)))
				if err != nil {
	...
	all_ready := false
	readyNodes := make(map[int]bool) // Map to keep track of nodes that are ready

	for !all_ready {
		all_ready = true // Assume all are ready until proven otherwise
		for validatorIdx, _ := range nodes {
			if validatorIdx != builderIdx {
				// Skip nodes that are already confirmed ready
				if readyNodes[validatorIdx] {
					continue
				}
				nodeReady := true
				for _, service := range testServices {
					targetStateDB := nodes[validatorIdx].getState()
					if targetStateDB != nil {
						code := targetStateDB.ReadServicePreimageBlob(service.ServiceCode, service.CodeHash)
						if len(code) == 0 {
							nodeReady = false
							break // Break out of the service loop for this node
						} else {
				...
				if nodeReady {
					readyNodes[validatorIdx] = true // Mark node as ready
				} else {
					all_ready = false
				}
			}
